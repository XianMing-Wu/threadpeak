...
- `packages/runtime-store` 的 duplicate/late/gap 原语是独立库能力；当前产品使用 SSE 快照推送加 revision 轮询恢复，未把内部事件诊断当作产品事件传输。
...
生成失败须区分传输、结构和内容质量。HTTP 200 且 JSON 通过 schema 后，不能因为缺少某个关键词而记为结构错误。R1 的学习选择和 R3 的条件题要求保留在当前 Agent 提示词中，程序不再用自然语言正则判断其是否合格。输出示例必须能通过对应 schema，避免示例数量或 ID 与校验相冲突。格式修复由程序执行且不重调模型，临时服务恢复仍有界；持续不可用或配置问题不能靠隐藏失败、无限重试或发布虚构结果解决。

聊天、卡片和完成事件在同一事务发布。嵌套事务使用同一连接 savepoint，PostgreSQL 与 PGlite 共用回滚行为测试。流式草稿不是成功结果；正文检查点和引用关联检查点分开，关联检查点恢复不清空或重播已经生成的正文。
...
任务快照在 SQL 中只读取界面所需字段，排除检查点和完整输入；资源列表按 (updated_at,id) 游标每页最多 100 个资源（资料目录单独每页 50 条，见下文），仅投影目录字段，客户端收齐后发布投影并防止账号切换时串入旧结果。作者来源在一次读取中建立节点、文章和原始段落索引，沿单父链复用结果；不跨资源版本缓存，删除与反馈撤销后重新读取。
...
活动资源通过同源 SSE 推送已提交的快照，路线使用变更信号唤醒读取。通知按 100 毫秒合并；正文仍按 dataRevision 去重，断线后从持久快照恢复，不依赖内存事件历史。每账号最多 4 条、单进程最多 400 条连接；流使用 64 KiB 高水位和背压，只保留待刷新的最新状态。15 秒心跳，60 秒结束连接并由浏览器重连以重新校验会话。此上限是资源保护措施，不是并发能力的压测结论。当前通知仅覆盖同进程写入，其他 worker 的提交由保底轮询发现；扩展多副本时需要共享通知通道。推送可用时前台每 5 秒校准一次；前端按帧平滑显示已收到的文字，发送状态先于网络请求呈现，历史回答和非活动画布避免随草稿重复渲染。
...
HTTP 按 owner/IP 分桶，登录和公开入口先按 IP 限流；可信代理用显式白名单，IPv6 规范化到 /64。默认每桶 600 次/分钟，数据库原子计数，窗口与清理使用数据库时间，多副本和重启共用。计数存储故障时 API 返回 503；轻量 `/health` 独立于数据库。共享计数增加数据库写入，实际吞吐必须压测。 PostgreSQL 建连超时 5 秒；共享 HTTP 计数超过 2.5 秒按存储不可用返回 503，迟到结果不能重新放行或重复响应，数据库故障时 readiness 不无限等待。
...
ops:queue 读取近 24 小时 provider、上下文与工作流分段指标：耗时分位数、失败/缓存命中、块数和摘要调用数；有实际 usage 才汇总。嵌套阶段不相加。模型输出的结构质量和语义质量分别验收，人工质量账本见[审查修复记录](https://github.com/XianMing-Wu/threadpeak/blob/6675d0511ff545770d48910bcdc3518eba2165ba/qa/project-audit-fixes-2026-09-08.md)。
...
上传使用独立 parser 作用域，收 body 前限制单实例最多 4 个、同账号最多 2 个上传，并按 Content-Length 预留合计最多 128 MiB 的 body 预算；未知长度按路由上限预留。超限在收取正文前返回 UPLOAD_BUSY，结束、错误和断连释放预算。PDF 从数据库读取字节并转传时另用一个共享名额，避免多个 100 MiB 文件同时解码和构造 multipart 副本；其余解析轮询仍共用知乎两路额度。请求超时 120 秒。每账号默认最多 500 份资料、1 GiB 来源字节预算，可由 THREADPEAK_MATERIAL_MAX_COUNT/THREADPEAK_MATERIAL_MAX_BYTES 调整，游客与知乎账号使用相同预算。导入在创建时预留 8 MiB 文本预算，避免异步完成突破已分配额度；这些数值不是物理数据库空间上限或生产压测结论。游客入口每 IP 每小时最多 20 次，仍受全局窗口限制。PDF 原始上传副本保留到解析正文和总结同一事务发布成功后删除；失败/取消保留恢复材料，远端 file_id 过期且未建立解析任务时可重新上传。远端解析成功后，在原 PDF 上运行有界 pdftotext（45 秒、8 MiB 输出、200 万字符），完整保留文字层并附上文字层中不存在的远端识别块，只忽略空白来去重，不删除运算符；记录独立检查点和正文来源。中文映射随镜像安装 poppler-data；远端失败或 90 秒仍未就绪，且本地确有至少 80 字时，按 Agent 合同保留真实文字层并标注扫描页、图片及公式缺失边界；仍无文字且无摘要时保留 PDF_UNREADABLE。
...
传输适配器原样保留正式 content，不选取内层 JSON，不把 reasoning 替作正文。L-answer 的数据解析上限 200 万字符，修复扫描嵌套上限 64；完整对象优先使用 jsonrepair 处理词法和分隔符；残缺输出仅提取已闭合的正文字符串，不猜写截断句子，保留非法反斜线的原意。标题通过 Markdown AST 降级，代码片段不改；续写字段合并与引用排序均在原数据上执行，不另发格式重写请求。未给出的语义、出处和被截断的正文不能由格式算法凭空恢复。
...
`POST /api/v2/workspace/clear` 需要当前身份、同源校验、幂等键、确认时冻结的工作区标识和代次以及明确确认体。事务按所有者删除活动数据和派生内容；仅保留无正文的清空回执及递增代次，避免请求超时后重放误删新数据。HTTP 和 worker 在进入时冻结代次，每个写事务以所有者共享 advisory lock 检查；清空使用独占锁并推进代次，外部网络等待不持锁，其他账号互不阻塞。全局 worker 调度退出请求或任务的写入上下文，防止被已清空账号的旧代次阻塞。清空后本机通知活动任务取消，其他实例的旧写入也由数据库代次阻断。保留身份与 OAuth 授权，既有运维备份仍遵守保留期，不把清空活动工作区宣称为立刻物理擦除所有备份。

浏览器清空当前账号草稿、投影、会话存储及 IndexedDB 账号备份，并重新打开首页以丢弃运行中的组件状态；其他账号备份不动。请求携带工作区代次，旧标签页或设备再次访问时清除过时代次投影。页面模块和封面在登录后空闲阶段及导航意图时预取，不创建隐藏 WebGL 上下文；书架首屏封面完成后才交给 3D 层显示，维持原生阅读入口。
