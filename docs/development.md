...
提交源码、可复用测试/QA 页面、当前文档、运行时必需资产与来源许可。`qa/evidence/` 默认忽略，只保留 QA 入口列出的必要公开账本；带日期的执行报告、截图、日志、模型载荷和媒体母版不再累积到主分支。停止跟踪不等于删除本机原件，历史证据通过固定提交回查。封面图库使用已发布的 WebP，PNG 母图与生成记录通过历史索引和哈希追溯。

文档审查区分当前规范、带日期的历史证据与第三方原文。本地 `.local-backend-review/` 的冻结副本、`刘看山/` 的创作工程和第三方依赖中的 Markdown 不属于现行产品规范，不应批量重写。更新当前计划书时同时保留其必要插图；更新维护合同后运行 `npm run check:docs`，并核对待发布文件集中的链接和资产依赖，不能仅因本地存在被忽略的文件就认为新克隆可用。
