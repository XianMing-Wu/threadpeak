...
10. **可见交互不由旧源码反推。** 路线题最多 3 轮；当前 active 题组全部选完后自动进入 R4，不出现「生成完整路径」；3D 边只表示推荐流转，不锁节点；分叉表示并列同时学，两路下一步接到同一节点；学习会话返回 3D，3D 返回路线列表；画布始终可“回到对话”；思考深度只有快速/深度两档且默认快速，学习页与画布共享，离开这一对界面后恢复快速，首页的深度不带入学习页；问博主没有首页快捷入口，也没有 Composer 模式入口，只有划选后才出现。某个界面流程开了深度时，该流程中的全部 LLM 与知乎直答都使用深度。Chat 没有发送上下文时进入独立 404。产品已删除图文模式，不得恢复图文快捷、前缀或失败页。首页普通发送走 R5，上下文是该 Chat 全文和只属于该 Chat 的附件，达到或超过 500k 才压缩。A2 的 `no_suitable_author` 只是程序分支，不得展示给用户。
...
as-implemented-logic.md 第 4.2 节与 agent-specs.md 第 7 节列出的空位仍未裁决，包括持久化表与事务、请求去重/取消/超时/刷新恢复、附件大小与生命周期、R5 失败重试与历史落盘、3D“上次位置”的具体数据、OAuth 可见状态和芯片文案。
