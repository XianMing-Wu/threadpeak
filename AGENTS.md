...
9. 登录提供知乎账号与游客双入口。游客经服务端隔离会话使用同一学习及持久流程，仅隐藏并禁止 OAuth 授权个人资料、收藏夹与创作导入；公开搜索和文件保留。OAuth/授权用户内容可按本轮明确裁决启用同接口本地 mock，标记演示并隔离账号，生产禁止。其他生产调用必须真实 provider；缺配置、服务不可用或结构不完整都不能用 fixture、计时器、固定作者、推测边或 reasoning 冒充成功。
