...
- 产品 `#session-learning` 在未选择 route/concept 时 fail-closed，不再默认 `linear-algebra` / `linear-map` 或发明首段讲解。`openLearning` 不再用 `defaultConceptId` 补第一个概念。已选中的进入仍是原型 lesson，不是 canonical 首答。
