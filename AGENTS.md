...
1. 路线只生成路线，R1 → 两路搜索 → R2 → R3/R3b 最多三轮 → R4 阶段计划 → 程序编译 → 3D 校验后发布；不建立学习图。当前题组答完自动生成，旧题保留且不可答。
...
12. 任务有持久检查点、所有者、幂等键、事件序号、租约及取消栅栏。刷新断连不取消任务，自动恢复不重复落结果。学习搜索三路，直答池两路；路线/问博主/作者搜索仍每组最多两路，不能丢问法。
