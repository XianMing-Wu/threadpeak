...
| --- | --- |
| 触发 | R2 完成后（无争议可跳过，极少） |
| 上下文获取 | 读取 R2 完整探索 JSON；读取这次路线请求原来绑定的全部附件。不给 R3 检索条目原文，也不给它最终路线 |
| 压缩算法 | 按 0.2。R2 的载体键、概念键和争议布尔值不可改、不可删；附件可压缩。需要压非附件时只压缩外围说明，探索 JSON 本体保持结构化 |

系统提示词：

> 你负责向完全不熟悉该领域的用户提出路线偏好选择题，不考知识。根据探索 JSON 中会实质改变学习路线的分歧，生成 1–3 道大白话问题，题数由你判断。问题要帮助用户说清自己想要什么或不想要什么，不能要求用户先懂术语、记定义或判断专业答案。每个选项都要写出它会怎样影响后续路线，供路径生成 Agent 使用。当前是第 1 轮；只输出指定 JSON。
...
| 上下文获取 | 读取用户这次追问原文与 activeRound；读取第 1 轮至当前轮的所有题组、每轮 active/superseded 状态、全部题目和选项、用户已经选择的 optionId；读取同一次路线请求绑定的附件 |
...
最多存在 3 轮题组。旧题及已答选项保留，并带 `superseded` 状态进入 R4；冲突时较新轮次优先。只有当前 active 题组全部答完才能进入 R4。
...
| --- | --- |
| 触发 | 当前套题全部选完。**不再检索** |
| 上下文获取 | 读取 R2 探索 JSON；读取所有轮次题组、active/superseded 状态和用户全部已选 optionId；读取每个选项的 routeEffect；读取本次路线请求绑定的附件及 sourceId。不再读取或发起知乎搜索 |
...
所有 ID 在单份输出内唯一；`carrierId/from*/to*` 必须引用已存在节点；图必须无环且所有节点从入口可达。数组顺序只负责稳定展示，一个节点多条出边表示分叉，多条入边表示汇合。
...
按 A1 问法并联检索。每条结果生成或取得稳定 evidenceId，并保留 queryId、authorId、作者展示名、API 文章总结和真实文章链接；相同作者的多篇内容仍各自保留 evidenceId，交给 A2 判断。
...
按 N1 问法并联检索，汇总 queryId、authorId、作者展示名、evidenceId、API 总结和链接；先排除 N0 已返回的 authorId，再按 authorId 合并同一人的多条证据。
