import bundledWasmUrl from "../../assets/wasm/rive.wasm?url";
import createRiveRuntime from "../../vendor/rive/canvas_advanced.mjs";
...
export async function loadRiveBytes(url, signal, kind = 'riv') {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(`Character ${kind} resource: HTTP ${response.status}`);
        const bytes = new Uint8Array(await response.arrayBuffer());
        const magic = kind === 'wasm' ? [0, 97, 115, 109] : [82, 73, 86, 69];
        if (!magic.every((value, i) => bytes[i] === value))
            throw new Error(`Invalid character ${kind} resource`);
        return bytes;
    }
    finally {
        clearTimeout(timer);
...
export function loadRiveRuntime(url = bundledWasmUrl) {
    ...
    if (!runtimes.has(key)) {
        ...
        pending.catch(() => { if (runtimes.get(key) === pending)
            runtimes.delete(key); });
