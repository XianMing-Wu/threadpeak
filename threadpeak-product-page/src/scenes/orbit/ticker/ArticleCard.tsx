...
function Icon({ name, size = 64, color = ink }: {
    name: string;
    size?: number;
    color?: string;
}) {
...
function VerifiedMark() {
    return <svg viewBox="0 0 64 64" width="52" height="52" aria-hidden="true"><polygon points="32,4 56,16 56,48 32,60 8,48 8,16" fill="#3d8bff"/><path d="M20 33l8 8 16-18" fill="none" stroke="#fff" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round"/></svg>;
}
export function ArticleCard({ data, width = 1080 }: {
  ...
  <div style={{ position: 'absolute', width: ARTICLE_WIDTH, height: ARTICLE_HEIGHT, transform: `scale(${scale})`, transformOrigin: 'top left', background: '#fff' }}>
   ...
   <div aria-hidden="true" style={{ position: 'absolute', left: 48, top: 294, width: 108, height: 108, borderRadius: '50%', background: topicColor[data.topic] ?? '#5b8fd4', color: '#fff', display: 'grid', placeItems: 'center', fontSize: 52, fontWeight: 600 }}>{data.author.slice(0, 1)}</div>
