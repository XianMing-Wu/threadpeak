...
import poster from "../../../assets/images/characters/reference.webp?url";
import sourceUrl from "../../../assets/rive/reference.riv?url";
...
export default function ReferenceCharacter({ onReady, reducedMotion = false }) {
    ...
    useEffect(() => {
        ...
        const look = event => { if (event.pointerType !== 'touch')
        ...
        const draw = time => {
            ...
            canvas.dataset.lookX = x.toFixed(3);
            canvas.dataset.lookY = y.toFixed(3);
            const dt = last && !reducedMotion && !document.hidden ? Math.min((time - last) / 1000, .05) : 0;
            last = time;
        ...
        const resume = () => { last = 0; if (!document.hidden && renderer && !frameId)
            frameId = runtime.requestAnimationFrame(draw); };
        ...
        const load = async (attempt = 0) => {
            ...
            try {
                ...
                if (!destroyed) {
                    ...
                    setStatus('error');
                    if (attempt === 0)
                        recoveryTimer = window.setTimeout(() => load(1), 1000);
                    else
                        console.warn('参考角色使用静态展示', error);
                }
            }
        };
        load();
        return () => {
            ...
            release();
            observer.disconnect();
            trackingArea.removeEventListener('pointerenter', look);
            trackingArea.removeEventListener('pointermove', look);
            trackingArea.removeEventListener('pointerleave', center);
