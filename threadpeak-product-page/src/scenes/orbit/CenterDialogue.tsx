...
function ThoughtDialogue({ goalMode = false, conversationLines }: {
    goalMode?: boolean;
    ...
    <svg className="thought-cloud" viewBox="0 0 416 416" aria-hidden="true" style={{ position: 'absolute', width: 360, height: 360, left: 388, top: 51, overflow: 'visible' }}>
      <g fill="white" stroke="#080808" strokeWidth="7.5" strokeLinejoin="round">
        ...
        <path d="M102 152C84 96 131 46 183 50C214 51 237 67 253 88C290 59 325 73 340 99C350 117 348 138 338 155C369 168 389 192 386 220C382 264 339 296 286 298C245 300 211 284 194 256C148 275 103 259 83 231C61 201 75 170 102 152Z" style={{ opacity: cloud, transformOrigin: '220px 260px', transform: `scale(${.75 + .25 * cloud})` }}/>
      </g>
...
export function CenterDialogue({ onComplete, reducedMotion, goalMode = false, conversationLines }: {
    onComplete: () => void;
    ...
    useEffect(() => {
        ...
        if (!current || !ready)
            return;
        const finish = () => { if (!completed.current) {
        ...
        const frame = ({ detail }: {
            ...
            host.current.dataset.frame = String(detail.frame); };
        current.addEventListener('ended', finish);
        current.addEventListener('frameupdate', frame);
        if (reducedMotion) {
            current.seekTo(DIALOGUE_DURATION - 1);
            finish();
        }
        else
            current.play();
    ...
    <Player ref={player} component={ThoughtDialogue} inputProps={inputProps} durationInFrames={DIALOGUE_DURATION} compositionWidth={600} compositionHeight={600} fps={60} initiallyMuted overflowVisible controls={false} loop={false} moveToBeginningWhenEnded={false} clickToPlay={false} doubleClickToFullscreen={false} spaceKeyToPlayOrPause={false} numberOfSharedAudioTags={0} style={{ position: 'absolute', inset: 0, width: '100%', pointerEvents: 'none' }}/>
  </div>;
}
