...
import { characterGoals,columnLayout,goals,mix,segment,type GoalId,type ScrollTransition } from "../../core/scroll-transition.ts";
import { PRELUDE_ORBIT,legacyRaw,pitchAt } from "../../core/story-clock.ts";
import { interviewLayout,interviewTurns,type StoryState } from "../interview/interview.ts";
import { choreographyAt,storyAt } from "../route/route-choreography.ts";
import { CenterDialogue } from "./CenterDialogue.tsx";
import { SpeechBubble,speechPath } from "./SpeechBubble.tsx";
import { characters,type CharacterPlayer } from "./characters.ts";
import { CHARACTER_SIZE,SCENE,SPEECH_HEIGHT,SPEECH_WIDTH,WIDE_SCENE,placeSpeech,portraitBox } from "./layout.ts";
import { ENTRANCE_DURATION,ORBIT_PERIOD,clamp,orbitPoint } from "./orbit.ts";
import { advanceSpeech,createSpeechCycle,speechEnvelope } from "./speech-cycle.ts";
function useReducedMotion() {
    const [value, setValue] = useState(() => matchMedia('(prefers-reduced-motion: reduce)').matches);
    useEffect(() => {
        const media = matchMedia('(prefers-reduced-motion: reduce)');
        const changed = () => setValue(media.matches);
        media.addEventListener('change', changed);
        return () => media.removeEventListener('change', changed);
    }, []);
    return value;
}
export function OrbitScene({ transition, frame, activeGoal, onGoal, onStory, beat }: {
    transition: RefObject<ScrollTransition>;
    frame: RefObject<HTMLDivElement | null>;
    activeGoal: GoalId;
    onGoal: (goal: GoalId) => void;
    onStory: (state: StoryState) => void;
    beat: number;
}) {
    ...
    const goalRef = useRef(activeGoal);
    ...
    useEffect(() => {
        ...
        const move = (event: PointerEvent) => { if (event.pointerType !== 'touch')
    ...
    useEffect(() => {
        ...
        const draw = (now: number) => {
            ...
            const wall = motion.progress, raw = legacyRaw(wall), p = Math.min(1, Math.max(0, raw)), q = segment(raw, 1.22, 1.74);
            const targetRaw = legacyRaw(motion.target);
            main.dataset.pitch = pitchAt(motion.target);
            const live = wall >= PRELUDE_ORBIT - .02 && raw < 7.4;
            if (live !== orbitLive) {
                orbitLive = live;
                if (live) {
                    cycle.current = createSpeechCycle();
                    setAutomatic(null);
                    phase.current = { angle: 0, entrance: 0 };
                    setComplete(false);
            ...
            if (storyKey !== lastStory) {
                lastStory = storyKey;
                onStoryRef.current(story);
            }
            main.dataset.learning = String(raw >= 9.41);
            main.dataset.canvas = String(raw > 9.60 && raw < 26.9);
            if (wall < PRELUDE_ORBIT) {
                stage.current!.inert = true;
                intro.inert = true;
                ticker.inert = true;
                goalScene.inert = true;
            ...
            if (raw >= 7.4) {
                ...
                main.style.setProperty('--interview-remain', '0');
                main.style.setProperty('--route-expand', '1');
                stage.current!.inert = true;
                stage.current!.setAttribute('aria-hidden', 'true');
                for (const hidden of [goalScene, intro, ticker]) {
                    hidden.inert = true;
                    hidden.setAttribute('aria-hidden', 'true');
                }
                // Keep the shared scroll clock alive without laying out hidden portraits.
                for (const player of players.current.values())
                    if (suspendedPlayers.get(player) !== true) {
                        player.pause();
                        suspendedPlayers.set(player, true);
                    }
                main.dataset.transition = raw.toFixed(4);
                id = requestAnimationFrame(draw);
                return;
            }
            stage.current!.inert = false;
            stage.current!.removeAttribute('aria-hidden');
            intro.inert = false;
            if ((raw > 1.58) !== interviewQuestionRef.current) {
                interviewQuestionRef.current = raw > 1.58;
                setInterviewQuestion(raw > 1.58);
            }
            if ((p > .48) !== goalQuestionRef.current) {
                goalQuestionRef.current = p > .48;
                setGoalQuestion(p > .48);
            }
            ...
            main.dataset.transition = raw.toFixed(4);
            main.dataset.scene = nextView;
            main.style.setProperty('--second-remain', String(1 - segment(raw, 1.22, 1.46)));
            main.style.setProperty('--interview-open', String(segment(raw, 1.36, 1.54)));
            main.style.setProperty('--interview-rise', `${20 * (1 - segment(raw, 1.36, 1.54))}px`);
            ...
            main.style.setProperty('--learner-x', `${interview.x}px`);
            main.style.setProperty('--learner-y', `${interview.y}px`);
            main.style.setProperty('--learner-size', `${interview.size}px`);
            ...
            goalScene.inert = p < .94 || raw > 1.22;
            goalScene.setAttribute('aria-hidden', String(p < .48 || raw > 1.56));
            intro.setAttribute('aria-hidden', String(p > .28));
            ticker.inert = p > .45;
            ticker.setAttribute('aria-hidden', String(p > .52));
            ...
            if (canRun) {
                ...
                if (!motion.locked && manualRef.current === null && !pausedRef.current && !reduced)
                    phase.current.angle += dt * Math.PI * 2 / ORBIT_PERIOD;
            }
            ...
            if (speechReady) {
                ...
                if (cycle.current.index !== cycle.current.displayed) {
                    cycle.current.displayed = cycle.current.index;
                    setAutomatic(cycle.current.index);
                }
            }
            ...
            nodes.current.forEach((node, i) => {
                ...
                if (links[i]) {
                    ...
                    starts[i].setAttribute('cx', String(wireStart));
                    starts[i].setAttribute('cy', String(rowY));
                }
                ...
                if (player && player.status === 'ready') {
                    ...
                    if (suspendedPlayers.get(player) !== suspended) {
                        if (suspended)
                            ...
                            player.play();
                        suspendedPlayers.set(player, suspended);
                        node.dataset.suspended = String(suspended);
                    }
                    ...
                    node.dataset.lookX = x.toFixed(3);
                    node.dataset.lookY = y.toFixed(3);
                }
            });
    ...
    return <section className="page" aria-label="对话之间：角色环绕" onPointerDown={event => { if (!(event.target as Element).closest('[data-character]'))
        setPinned(null); }} onKeyDown={event => { if (event.key === 'Escape') {
        setPinned(null);
        setFocus(null);
        setHover(null);
    ...
    <div className="hero-copy">
      <h1>先说出目标，<br />再选学习路线。</h1>
      <p>同一件事会有很多条学习路线。<br />问山第一件事，是先选出适合你的一条。</p>
    </div>
    ...
    <div className="orbit-stage" ref={stage} data-phase={complete ? 'orbit' : 'dialogue'} data-ready-count={ready.length}>
      ...
      <div className="characters" aria-label="六种不同的视角" aria-hidden={!orbitVisible}>
        {characters.map(({ id, label, Component, message }, i) => <button key={id} type="button" ref={node => { nodes.current[i] = node; }} className={`orbit-character${active === i ? ' is-active' : ''}`} aria-label={view === 'column' ? `${label}，查看${goals.find(goal => goal.id === characterGoals[i])!.title}目标` : `${label}，查看对话`} aria-describedby={view === 'column' ? `answer-${id}` : active === i ? `speech-${id}` : undefined} aria-expanded={view === 'column' ? undefined : active === i} tabIndex={orbitVisible ? 0 : -1} disabled={!orbitVisible} data-character={id} data-ready={ready.includes(i)} data-load-state={errors[i] ? 'fallback' : ready.includes(i) ? 'ready' : 'loading'} onPointerEnter={event => { if (view === 'column')
            onGoal(characterGoals[i]);
        else if (view === 'orbit' && event.pointerType !== 'touch')
            setHover(i); }} onPointerLeave={() => setHover(null)} onFocus={event => { if (view === 'column')
            onGoal(characterGoals[i]);
        else if (event.currentTarget.matches(':focus-visible'))
            setFocus(i); }} onBlur={() => { setFocus(null); setPinned(null); }} onClick={event => { if (view === 'column')
            onGoal(characterGoals[i]);
        else if (event.detail === 0 || matchMedia('(pointer: coarse)').matches)
            setPinned(pinned === i ? null : i); }}>
          <span className="character-portrait"><Component key={run} size="100%" follow="none" onReady={player => {
                setErrors(values => { const next = { ...values }; delete next[i]; return next; });
                players.current.set(i, player);
                setReady(values => values.includes(i) ? values : [...values, i]);
