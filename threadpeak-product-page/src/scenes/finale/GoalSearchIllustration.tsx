...
import { goalTypingAt } from "./goal-typing.ts";
export const goalExamples = [
    { goal: '从零训出一个能跑完、能验收的小模型', steps: ['规模与预算', '结构与词表', '训练与验收'] },
    { goal: '拍好一组能讲故事的旅行照片', steps: ['观察光线', '练习构图', '编排组照'] },
    { goal: '三年内买房前先看清现金流', steps: ['期限', '风险底线', '比较方案'] },
    { goal: '做出一份能拿去面试的设计作品', steps: ['目标用户', '界面', '作品评审'] },
    { goal: '系统了解明代内阁如何运作', steps: ['制度', '人物', '史料'] },
] as const;
/** Complete characters, a measured caret and the route use the same clock. */
export function GoalSearchIllustration({ example, active = true }: {
    example: number;
    active?: boolean;
}) {
    ...
    useEffect(() => {
        ...
        const el = host.current!, motion = matchMedia('(prefers-reduced-motion: reduce)');
        let frame = 0, time = 0, last = 0, painted = -1;
        const draw = (now: number) => {
            ...
            if (last)
                time += Math.min(.05, (now - last) / 1000);
            last = now;
            ...
            el.dataset.typingPhase = s.phase;
            el.style.setProperty('--goal-results', String(s.results * s.fade));
            el.style.setProperty('--goal-wire', String((1 - s.results) * 100));
            el.style.setProperty('--goal-fade', String(s.fade));
            el.style.setProperty('--goal-press', String(1 - s.press * .12));
            if (caret.current)
                caret.current.style.opacity = s.caret ? '1' : '0';
            if (!motion.matches)
                frame = requestAnimationFrame(draw);
        };
        const resume = () => { cancelAnimationFrame(frame); last = 0; if (!document.hidden)
            frame = requestAnimationFrame(draw); };
        frame = requestAnimationFrame(draw);
    ...
    useLayoutEffect(() => { let alive = true; const measure = () => { if (alive && text.current && caret.current) {
        const width = text.current.getComputedTextLength?.() ?? 0;
        caret.current.setAttribute('transform', `translate(${Math.min(298, width + 2)} 0)`);
    } }; measure(); void document.fonts?.ready.then(measure); return () => { alive = false; }; }, [count, active]);
