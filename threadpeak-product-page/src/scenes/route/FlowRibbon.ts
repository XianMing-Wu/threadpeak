...
import { guideDistance,guideSegments,sampleGuide } from "./route-guide.ts";
/** White core, cyan body, blue corona and fine filaments from 光源流动.mov. */
export class FlowRibbon {
    readonly group = new T.Group();
    private readonly strips: Array<{
    ...
    constructor() {
        ...
        for (const d of definitions) {
            ...
            for (let i = 0; i <= this.count; i++) {
                uvs.set([0, i / this.count, 1, i / this.count], i * 4);
                if (i < this.count) {
                    const n = i * 2;
      ...
      gl_FragColor=vec4(blue,a*fade*opacity*pulse);}` });
            ...
            mesh.renderOrder = 15 + d.layer;
            this.group.add(mesh);
            this.strips.push({ ...d, mesh });
        }
    }
    update(raw: number, opacity: number) {
        ...
        for (const strip of this.strips) {
            strip.mesh.material.uniforms.opacity.value = opacity;
            strip.mesh.material.uniforms.pulse.value = .93 + .07 * Math.sin(raw * 67) ** 2;
            ...
            for (let i = 0; i <= this.count; i++) {
                ...
                attribute.setXYZ(i * 2, a.x + nx * (noise - width / 2), y, a.z + nz * (noise - width / 2));
                attribute.setXYZ(i * 2 + 1, a.x + nx * (noise + width / 2), y, a.z + nz * (noise + width / 2));
            }
