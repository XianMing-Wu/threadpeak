...
function around(index: number, direction: number) {
    ...
    while ((b - a) * direction < Math.PI * 2)
        b += direction * Math.PI * 2;
    ...
    const points: Point[] = [];
    for (let i = 0; i <= 24; i++)
        points.push(bezier(entry, add(entry, before, -.40), add(first, tangentA, -.27), first, i / 24));
    for (let i = 1; i <= 180; i++)
        points.push(add(center, polar(a + (b - a) * i / 180, radius)));
    for (let i = 1; i <= 24; i++)
        points.push(bezier(last, add(last, tangentB, .27), add(exit, after, -.40), exit, i / 24));
...
for (let i = 0; i < 4; i++) {
    rawSegments.push({ carrier: null, points: [previous, circles[i].entry] }, { carrier: i + 1, points: circles[i].points });
    previous = circles[i].exit;
}
...
export function sampleGuide(distance: number): Point {
    if (distance <= 0)
        return { ...samples[0] };
    if (distance >= total) {
        const last = samples[samples.length - 1], dir = unit(sub(last, samples[samples.length - 2]));
        return add(last, dir, distance - total);
    }
    ...
    return mix(a, b, (distance - a.distance) / (b.distance - a.distance));
}
