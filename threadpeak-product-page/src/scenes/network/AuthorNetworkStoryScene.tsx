...
function Paper({ id, w, h, label, children }: {
    id: string;
    w: number;
    h: number;
    label: string;
...
export function AuthorNetworkStoryScene({ transition }: {
        ...
        function update() {
            ...
            if (visible && !document.hidden && width) {
                if (dirty || lastRaw !== raw || lastSelected !== active.current) {
                    ...
                    stage.style.width = `${fit.W * fit.scale}px`;
                    stage.style.height = `${fit.H * fit.scale}px`;
                    stage.style.left = `${fit.x}px`;
                    stage.style.top = `${fit.y}px`;
                    ...
                    opacity(viewport, networkVolumeProgress(p) > .525 ? 1 : 0);
                    bridges.forEach((n, i) => {
                    ...
                    el.querySelector<SVGGElement>('.network-footnote')!.setAttribute('transform', `translate(${fit.narrow ? 40 : 778} ${fit.narrow ? 1418 : 786})`);
                    opacity(el.querySelector<SVGGElement>('.network-footnote')!, part(p, .96, .972));
  ...
  <svg className="author-stage" viewBox="0 0 1400 820">
   ...
   <Paper id="source" {...footprintBoxes.source} label="关联的知乎文章"><div className="footprint-kicker"><FootprintMark kind="source"/> 来源文章</div><h3>{sourceLead && <span className="footprint-title-topic">{sourceLead}</span>}{sourceRest}</h3><p className="footprint-source-meta">{f.author.name} · {likes.toLocaleString('zh-CN')} 赞同</p><p className="footprint-source-excerpt">{excerpt}</p></Paper>
   <Paper id="concept" {...footprintBoxes.concept} label="当前项目的学习任务"><div className="footprint-kicker"><FootprintMark kind="concept"/> 我的目标</div><h3>{arrivingAuthors.includes(selected) ? '从零训出一个能验收的小模型。当前停在规模：会定计算最优，还没选定注意力实现。' : f.group.title.split('：').slice(-1)}</h3></Paper>
