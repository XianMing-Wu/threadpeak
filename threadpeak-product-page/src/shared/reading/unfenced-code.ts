...
export function fenceUnfencedCode(source: string): string {
    ...
    source = source.replace(/^(\s*)(\$\$?|\\\()[ \t]*(.*?)[ \t]*(?:\$\$?|\\\))[ \t]*$/gm, (line, indent, _delimiter, content, offset) => !literalRanges.some(([start, end]) => start <= offset && end > offset) && codeStart(content) ? indent + content : line);
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (/^(?: {4}|\t)/.test(lines[i]!) || protectedLine(i) || !(codeStart(lines[i]!) || startsArray)) {
            result.push(lines[i]!);
            continue;
        }
        const block: string[] = [];
        const cpp = cppStart.test(lines[i]!) || cppCall.test(lines[i]!);
        if (cpp && /^\s*\/\//.test(result.at(-1) ?? ''))
            block.push(result.pop()!);
        const stack: string[] = [];
        ...
        const scan = (line: string) => {
            for (const c of (cpp ? line.replace(/\/\/.*$/, '') : line)) {
                ...
                if ('([{'.includes(c))
                    stack.push(c);
                if (')]}'.includes(c) && '([{'.indexOf(stack.at(-1) ?? '') === ')]}'.indexOf(c))
                    stack.pop();
            }
        };
        let end = i;
        for (; end < Math.min(lines.length, i + 128) && !protectedLine(end); end++) {
            ...
            if (end !== i && !stack.length && !quote && !codeStart(line) && !codeContinuation.test(line) && !comment.test(line) && !continuedCode && !(cpp && /^\s*\{\s*$/.test(line)))
                break;
            // A truncated array must not swallow the explanation that follows it.
            if (end !== i && (stack.length || quote) && /^[\u3400-\u9fff]/.test(line.trim()))
                break;
            block.push(line);
            scan(line);
        }
