import {assetUrl,isBundledAssetUrl} from '../../runtime/assets';
import type { Definition,RootContent } from 'mdast';
import { fromMarkdown } from 'mdast-util-from-markdown';
import { decodeMathEncoding,normalizeTex,renderMath } from "./math-normalize.ts";
...
export function safeSourceImageUrl(value: string | undefined): string | undefined {
    if (isBundledAssetUrl(value)) return value;
    if (value?.startsWith('assets/') && !value.includes('..')) {
        try { return assetUrl(value); } catch { return; }
    }
    if (!value || value.length > 8192 || /[\s\\\u0000-\u001f\u007f]/.test(value) || /%(?![\da-f]{2})/i.test(value))
        return;
    try {
        const url = new URL(value, document.baseURI);
        if (url.origin === location.origin && url.pathname.startsWith('/assets/images/authors/'))
            return url.href;
        ...
        if (!host.includes('.') || host.endsWith('.') || host.startsWith('[') || /^[\d.]+$/.test(host)
            || /(?:^|\.)(?:localhost|local|internal|intranet|lan|home|corp|test|invalid)$/.test(host))
            return;
        return url.href;
    }
    catch {
...
function attributes(tag: string): Record<string, string> {
    ...
    for (const match of body.matchAll(pattern)) {
        ...
        if (!(key in values))
            values[key] = decodeAttribute(match[2] ?? match[3] ?? match[4] ?? '');
    }
    return values;
}
...
function imageMarkdown(image: ImageSource): string {
    ...
    if (info.tex && renderMath(info.tex, false).kind === 'rendered')
        return `$${info.tex}$`;
...
function htmlImages(html: string): string {
    ...
    for (const match of html.matchAll(tagPattern)) {
        ...
        if (blocked.length) {
            if (closing && name === blocked.at(-1))
                blocked.pop();
            else if (!closing && inertContainers.has(name) && !/\/>$/.test(token))
                blocked.push(name);
            continue;
        }
        if (inertContainers.has(name)) {
            if (!closing && !/\/>$/.test(token) && name !== 'embed')
                blocked.push(name);
            continue;
        }
        if (name === 'img' && !closing) {
        ...
        else if (name === 'br' || /^(?:p|div|section|article|h[1-6]|li|blockquote|figure|figcaption)$/.test(name))
            output += '\n\n';
    ...
    const protectHtml = (node: RootContent) => {
        ...
        if (node.type === 'html') {
            for (const token of node.value.matchAll(tagPattern)) {
                ...
                if (!name || (!inertContainers.has(name) && name !== 'code' && name !== 'pre'))
                    continue;
                const start = (node.position?.start.offset ?? 0) + token.index!;
                if (htmlLiterals.some(([a, b]) => start >= a && start < b))
                    continue;
            ...
            node.children.forEach(protectHtml);
    };
    ast.children.forEach(protectHtml);
    const walk = (node: RootContent) => {
        const start = node.position?.start.offset, end = node.position?.end.offset;
        if (start == null || end == null || node.type === 'code' || node.type === 'inlineCode')
            return;
        if (htmlLiterals.some(([a, b]) => start >= a && end <= b))
            return;
        if (node.type === 'image' || node.type === 'imageReference') {
            const definition = node.type === 'imageReference' ? definitions.get(node.identifier) : node;
            if (definition)
                edits.push({ start, end, value: imageMarkdown({ src: definition.url, alt: node.alt ?? '', title: definition.title ?? '' }) });
            return;
        }
        if (node.type === 'html') {
            ...
            for (const [a, b] of htmlLiterals.filter(([a, b]) => a < end && b > start).sort((a, b) => a[0] - b[0])) {
                if (a > cursor)
                    edits.push({ start: cursor, end: a, value: htmlImages(source.slice(cursor, a)) });
                cursor = Math.max(cursor, b);
            }
            if (cursor < end)
                edits.push({ start: cursor, end, value: htmlImages(source.slice(cursor, end)) });
            return;
        }
        if ('children' in node)
...
export function protectedSourceRanges(source: string, options: {
    ...
    const walk = (node: RootContent) => {
        if (node.type === 'code' && options.indentedCode === false && !/^ {0,3}(?:`{3,}|~{3,})/.test(source.slice(node.position?.start.offset ?? 0)))
            return;
...
export function sourceExcerptIssues(source: string): SourceExcerptIssue[] {
    ...
    rules.push(['missing-operands', /(?:例子(?:是)?|表达式(?:是)?|函数(?:为|是)?)[：:][ \t\u00a0\u3000]*[，,]/], ['missing-operands', /(?:计算|求导(?:得到|得)?)[ \t\u00a0\u3000]*[，,][ \t]*(?:假设|设|给定)/], ['missing-operands', /返回值就是[ \t\u00a0\u3000]{2,}这一梯度/], ['empty-formula-list', /手算的话[，,][ \t\u00a0\u3000]*[，,][ \t\u00a0\u3000]*[，,]/]);
    return [...new Set([...rules.filter(([, pattern]) => pattern.test(prose)).map(([code]) => code), ...(emptyMath ? ['empty-formula-list' as const] : [])])].map(code => ({ code }));
}
export type SourceExcerptIssue = {
    code: 'missing-operands' | 'empty-formula-list' | 'empty-math-law';
};
