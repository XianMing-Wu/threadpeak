...
import { mathFencesToDelimiters,prepareSourceImages,protectedSourceRanges } from "./source-image.ts";
import { fenceUnfencedCode } from "./unfenced-code.ts";
export type PreparedMath = {
    tex: string;
    display: boolean;
};
...
export function isShortInlineMath(tex: string) {
    ...
    if (!compact)
        return false;
    if (/\\begin|\\tag|\\\\/.test(tex))
        return false;
    return !tex.includes('\n') && compact.length <= 48;
}
function splitFences(source: string): Array<{
    code: boolean;
    text: string;
}> {
    const parts: Array<{
    ...
    const prose = (text: string) => {
        for (const block of text.split(/(https?:\/\/[^\s<>]+|[A-Za-z]:\\[^\s]+)/g)) {
            if (block)
                parts.push({ code: /^(?:https?:|[A-Za-z]:\\)/.test(block), text: block });
        }
    };
...
function wrapAsInline(chunk: string) {
    ...
    if (!tex || tex.includes('%%TPMATH'))
        return chunk;
    if (/^\\(?:left|right)$/.test(tex))
        return chunk;
    return `$${tex}$`;
}
...
function wrapJoinedMatrixLines(text: string) {
    return text.replace(/[^\n]+/g, (line) => {
        if (line.includes('%%TPMATH') || !/[,;][ \t]*\\?(?:qquad|quad)\b/.test(line))
            return line;
        if (!/\\begin\{(?:matrix|pmatrix|bmatrix|vmatrix)\}[\s\S]*\\end\{(?:matrix|pmatrix|bmatrix|vmatrix)\}/.test(line))
            return line;
...
function protectDelimited(text: string, slots: string[]) {
    return text.replace(/\$\$([\s\S]+?)\$\$|\\\[([\s\S]+?)\\\]|(?<!\\)\$([^$\n]+?)(?<!\\)\$|\\\(([^]*?)\\\)/g, (_all, displayA, displayB, inlineA, inlineB, offset) => {
        if (inlineA != null && /^(?:\d[\d.,]*\s+(?:and|or|USD|美元|元|和|或|至|到)|.*\b(?:dollars|price)\b)/i.test(inlineA))
            return _all.replace(/\$/g, '\\$');
...
function mergeAdjacentSlots(text: string, slots: string[]) {
    return text.replace(/%%TPMATH\d+%%(?:[ \t]*(?:[=+−\-<>≤≥≠][ \t]*)?%%TPMATH\d+%%)+/g, (run) => {
        ...
        if (!values.every((value) => isInlineDollar(value)))
            return run;
        const index = slots.length;
...
function wrapProbability(text: string) {
    return text.replace(/P\s*(?:\\left\s*)?\((?:[^()%]|\([^()%]*\)){1,220}\)(?:\s*\\right)?/g, wrapAsInline);
}
...
function wrapBracedTex(text: string) {
    return text.replace(/\{(?=[^{}]*[_^])(?:[^{}]|\{[^{}]*\})+\}(?:\\in\s*\[[^\]\n]+\])?/g, (chunk) => wrapAsInline(chunk.replace(/^\{([\s\S]*)\}/, '$1')));
}
function wrapTexScripts(text: string) {
    return text.replace(/(?<![A-Za-z\\%])[A-Za-z][A-Za-z0-9]*(?:_\{[^{}]+\}|\^\{[^{}]+\})+[A-Za-z0-9]*(?:\\in\s*(?:\[[^\]\n]+\]|\\left[\s\S]{0,80}?\\right\s*[).\]|]))?/g, wrapAsInline);
}
...
function normalizeXrightarrow(text: string) {
    return text.replace(/\\xrightarrow(?:\s*)(?!\{)([A-Za-z][A-Za-z0-9]*(?:_[A-Za-z0-9]+)?)/g, '\\xrightarrow{$1}');
}
function wrapTransitionChains(text: string) {
    return text.replace(/[A-Za-z][A-Za-z0-9]*(?:_[A-Za-z0-9]+)?(?:\s*\\xrightarrow\{[^{}]*\}(?:\s*[A-Za-z][A-Za-z0-9]*(?:_[A-Za-z0-9]+)?|\.{2,})?)+/g, wrapAsInline);
}
function wrapCommandRuns(text: string) {
    return text.replace(/(?:[A-Za-z][A-Za-z0-9]*)?(?=\\(?:times|rightarrow|in|mathbb|cdot|ldots|to|subset|subseteq))(?:\\[a-zA-Z]+(?:\s*\{[^{}]*\})?|\\[^a-zA-Z\s$%])+(?:\s*(?:\\[a-zA-Z]+(?:\s*\{[^{}]*\})?|\\[^a-zA-Z\s$%]|\{[^{}]+\}|\[[^\]\n]+\]|[A-Za-z][A-Za-z0-9]*))*|(?:\\[a-zA-Z]+(?:\s*\{[^{}]*\})?|\\[^a-zA-Z\s$%])+(?:\s*(?:\\[a-zA-Z]+(?:\s*\{[^{}]*\})?|\\[^a-zA-Z\s$%]|\{[^{}]+\}|\[[^\]\n]+\]|[A-Za-z][A-Za-z0-9]*))*/g, wrapAsInline);
}
...
function collectDelimitedMath(markdown: string): PreparedMath[] {
    ...
    const walk = (node: ReturnType<typeof readingAst>['children'][number]) => {
        if (node.type === 'math' || node.type === 'inlineMath')
            result.push({ tex: node.value.trim(), display: node.type === 'math' });
...
function prepareText(text: string) {
    ...
    const connected = islands.replace(/((?:\[[A-Za-z][A-Za-z0-9]*\]|[A-Za-z])[A-Za-z0-9_{}^'\[\](). +\-]*=[ \t]*)(%%TPMATH\d+%%)/g, (all, lhs, slot) => {
        ...
        if (!value || !isInlineDollar(value) || /[A-Za-z]{3,}/.test(lhs))
            return all;
        const index = slots.length;
        slots.push(`$${lhs}${value.slice(1, -1)}$`);
        return SLOT(index);
    });
...
export function replaceMathPlaceholders(text: string, math: readonly PreparedMath[], render: (item: PreparedMath, index: number) => unknown) {
    const pattern = /\$\$([\s\S]+?)\$\$|(?<!\\)\$([^$\n]+?)(?<!\\)\$/g;
    const parts: unknown[] = [];
    let last = 0;
