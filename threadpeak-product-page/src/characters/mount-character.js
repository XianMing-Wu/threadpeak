import { loadRiveBytes,loadRiveRuntime } from "../runtime/rive-loader.js";
const clamp = (value, min, max) => Math.max(min, Math.min(max, value));
/** Mount into a DOM container. Call destroy() when the host is unmounted. */
export function createCharacterMount(character, sourceUrl) {
    return function mountCharacter(target, options = {}) {
        const host = typeof target === 'string' ? document.querySelector(target) : target;
        if (!(host instanceof HTMLElement))
            throw new TypeError('mountCharacter requires a DOM container.');
        const follow = options.follow ?? 'page';
        if (!['page', 'canvas', 'none'].includes(follow))
            throw new TypeError('follow must be page, canvas or none.');
        const fit = options.fit ?? 'contain';
        if (!['contain', 'cover'].includes(fit))
            throw new TypeError('fit must be contain or cover.');
        ...
        function resize() {
            ...
            lastDpr = dpr;
            if (clientPoint)
                pointFromClient(clientPoint.x, clientPoint.y);
            schedule();
        }
        ...
        function schedule() {
            if (!destroyed && runtime && renderer && !frameId)
        ...
        function draw(time) {
            try {
                ...
                if (destroyed)
                    return;
                if (lastDpr !== clamp(window.devicePixelRatio || 1, 2, 3))
                    resize();
                if (pointerDirty) {
                ...
                if (viewportWidth && viewportHeight) {
                    renderer.clear();
                    renderer.save();
                    renderer.align(fit === 'cover' ? runtime.Fit.cover : runtime.Fit.contain, runtime.Alignment.center, { minX: 0, minY: 0, maxX: canvas.width, maxY: canvas.height }, bounds);
        ...
        function visibilityChanged() { lastTime = 0; if (!document.hidden)
            resize(); }
        function motionChanged() { lastTime = 0; if (reduced())
            center(); schedule(); }
        function scrolled() {
            if (clientPoint && !reduced()) {
                pointFromClient(clientPoint.x, clientPoint.y);
                schedule();
            }
        }
        ...
        if (follow !== 'none')
            eventTarget.addEventListener('pointermove', onPointer, { passive: true });
        if (follow === 'canvas')
        ...
        const api = {
            ...
            lookAt(x, y) {
                ...
                if (!Number.isFinite(x) || !Number.isFinite(y))
                    throw new TypeError('lookAt requires finite numbers.');
                clientPoint = null;
        ...
        api.ready = (async () => {
            try {
                const [rt, bytes] = await Promise.all([
                    ...
                    loadRiveBytes(options.src ?? sourceUrl, abort.signal),
                ]);
                if (destroyed)
                    throw new DOMException('Character unmounted', 'AbortError');
                runtime = rt;
                ...
                file = loaded;
                if (!file)
                    throw new Error('Unable to decode character.riv.');
                if (file.artboardCount() !== 1)
                    throw new Error('Expected a single-character Rive file.');
                artboard = file.artboardByName(character.artboard);
                if (!artboard)
                    throw new Error(`Missing artboard: ${character.artboard}`);
                const definition = artboard.stateMachineByName(character.stateMachine);
                if (!definition)
                    throw new Error(`Missing state machine: ${character.stateMachine}`);
                machine = new runtime.StateMachineInstance(definition, artboard);
                renderer = runtime.makeRenderer(canvas);
                if (!renderer)
                    throw new Error('Character renderer unavailable.');
                artboard.advance(0);
                resize();
                if (frameId) {
                    runtime.cancelAnimationFrame(frameId);
                ...
                draw(0);
                if (destroyed)
                    throw new Error('Character first frame failed.');
                options.onReady?.(api);
                return api;
            }
            catch (error) {
