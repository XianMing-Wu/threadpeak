...
export function createCharacterView(mountCharacter, character, poster) {
    return function Character({ size = 320, follow = 'page', autoplay = true, respectReducedMotion = true, src, wasmUrl, className, style, onReady, onError, ...props }) {
            ...
            const start = () => {
                ...
                player = mountCharacter(host.current, { follow, autoplay, respectReducedMotion, src, wasmUrl,
                    ...
                    onError: error => {
                        ...
                        if (attempts < 2 && navigator.onLine !== false)
                            timer = setTimeout(start, 1000);
                    },
                });
            };
            const online = () => { if (phase === 'fallback' && attempts < 2)
                start(); };
        ...
        return createElement('div', { ...props, className, 'data-character-state': status,
            style: { width: size, height: size, position: 'relative', ...style } }, createElement('img', { src: poster, alt: `${character.label}（静态展示）`, draggable: false,
            style: { position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'contain', display: status === 'ready' ? 'none' : 'block' } }), createElement('div', { ref: host, 'aria-hidden': status !== 'ready',
