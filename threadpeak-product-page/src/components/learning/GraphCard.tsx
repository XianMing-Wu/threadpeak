...
export function GraphCard({ node, selected = false, expanded = false, onExpand, onOriginal, onSource, editing, onFinishEdit, toolbar = true, ...actions }: {
    ...
    const expandLabel = expanded ? '收起内容' : node.type === 'custom' ? '展开原文' : node.type === 'article' && example ? '展开阅读说明' : node.type === 'article' && articles.find(a => a.id === node.id)?.sourceKind !== 'upload' ? '展开完整总结' : '展开完整正文';
    return <div className={`lp-graph-card ${selected ? 'is-selected' : ''}`} data-type={node.type} data-custom-color={node.color ? true : undefined} style={{ background: node.color ?? 'var(--card-fill, var(--surface-1))', borderWidth: node.stroke ?? undefined }}>
