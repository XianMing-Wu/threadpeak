...
import { cardColors } from "./card-colors.ts";
import { cycleDialogFocus,useDialogFocus } from "./dialog-focus.ts";
...
export function NodeEditor({ mode, node, onMode, onClose, onAdd, onEdit, onColor, onStroke, onDuplicate, onDelete }: {
    ...
    return <section ref={dialog} className="lp-node-editor" role="dialog" aria-label={labels[mode]} onPointerDown={e => e.stopPropagation()} onWheel={e => e.stopPropagation()} onKeyDown={e => { cycleDialogFocus(e); e.stopPropagation(); if (e.key === 'Escape')
        onClose(); }}>
