...
const selection=e=>({sourceRef:typeof e==='string'?e:e.ref,fit:'direct',reason:'材料讲解了换基与坐标之间的关系。',canHelpWith:'理解换基',limitation:'未确认是否提供咨询',question:'能否带我算一个二维例子？',passageRefs:['P1']})
...
test('matching selects numbered passages, preserves all text and never invents identity after compression',()=>{
 const a=source('a'),b=source('b'),original=[a,b],candidates=authorEvidenceScope(original)
...
test('relevance precedes preference, personalization is explainable and hidden/new controls remain effective',()=>{
 ...
 const draft=consultationDraft(AuthorBriefSchema.parse({question:'如何换基',purpose:'consult',attempted:'做了一个例子'}),items[0])
...
test('author feedback validates ownership and source/topic membership, is reversible and idempotent',async t=>{
 ...
 const session=await app.inject({url:'/api/v2/session'}),cookie=session.headers['set-cookie'].split(';')[0],headers={cookie},owner=(await store.db.query('SELECT owner_id FROM tp_sessions'))[0].owner_id
...
test('search and import enforce ownership and retry the original frozen command after source edits',async t=>{
 ...
 const accepted=await store.existingCommand(owner,'frozen-search');assert.equal(accepted.input.selectedContext[0].content,source('a').summary)
...
test('importing the second article of one author adds only that article to the selected concept',async t=>{
 ...
 const session=await app.inject({url:'/api/v2/session'}),headers={cookie:session.headers['set-cookie'].split(';')[0]},owner=(await store.db.query('SELECT owner_id FROM tp_sessions'))[0].owner_id
