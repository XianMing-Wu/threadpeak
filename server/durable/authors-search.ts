...
export async function searchAuthors(ctx:TaskContext,tools:ProductTools){
  ...
  const input={currentQuestion:brief.question,purpose:brief.purpose,background:{content:brief.background},attempted:{content:brief.attempted},desiredOutcome:{content:brief.desiredOutcome},selectedContext:ctx.job.input.selectedContext??[]}
