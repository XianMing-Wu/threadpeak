import { ANSWER_COMPLETENESS,COMPOSE_PROMPT,COMPOSE_OUTPUT,ATTACH_PROMPT,ATTACH_OUTPUT,validateComposition,citationCatalog,attachComposition } from '../knowledge/answer-composition.ts'
...
import { z } from 'zod'
import { AGENT_PROMPTS, systemPromptFor } from '../agent-runtime/prompts.ts'
...
import type { LlmProvider, ZhihuProvider, L0aAngle, ThinkingDepth, SearchEvidence } from '../agent-runtime/types.ts'
...
export class ProductTools {
  ...
  async structured<T>(ctx:TaskContext, name:string, system:string, input:unknown, validate:(value:unknown,prepared:unknown)=>T, output=8192, options:{stream?:boolean;prepare?:(input:any)=>unknown}={}):Promise<T> {
    const runtime={system,budget:this.capabilities.llm,formalOutput:2}
    return ctx.step(`${name}@${AGENT_CONTRACT_VERSION}:${digest(runtime).slice(0,16)}`, {input,...runtime}, async()=>{
      ...
      const base=await packContext(this.llm,ctx,system,input,depth,{window:this.window,output:reserve,margin:options.prepare?Math.max(4096,Math.ceil(window*.08)):2048,summary:this.capabilities.llm})
  ...
  async learning<T extends LearningTool>(ctx:TaskContext,name:T,input:unknown,validate?:(v:any)=>void,checkpoint:string=name):Promise<z.infer<(typeof LEARNING_TOOL_SPECS)[T]['schema']>>{
    ...
    return this.structured(ctx,name==='A-card-plan'?`${checkpoint}:v2`:checkpoint,`${SHARED_SYSTEM_PREFIX}\n${GOAL_POLICY}\n${spec.prompt}\n输出 JSON：${spec.output}`,input,v=>{
      const parsed=spec.schema.parse(v);if(name==='A-card-plan')packAuthorSearchQueries((parsed as {queries:string[]}).queries);validate?.(parsed);return parsed as z.infer<(typeof LEARNING_TOOL_SPECS)[T]['schema']>
  ...
  async planStep<T>(ctx:TaskContext,agent:RouteStep,input:unknown,parseInput:ParseAgentOutputInput={},checkpoint:string=agent):Promise<T>{
    ...
    return this.structured(ctx,checkpoint,prompt,input,v=>{
      ...
      if(agent==='R1')packZhihuSearchQueries((parsed.value as {queries:{id:string;text:string;angle?:string}[]}).queries)
  ...
  async routePlan(ctx:TaskContext,input:{attachments?:{sourceId:string;fileName:string;content:string;contentBasis?:string}[];[key:string]:unknown},scope:string,attachmentSourceIds:string[]):Promise<R4Output>{
    ...
    const planned=await this.structured(ctx,'R4-plan:strict-v2',`${SHARED_SYSTEM_PREFIX}\n${GOAL_POLICY}\n${STAGED_PLAN_PROMPT} searchScope.kind=collections 时只根据所选资料安排学习，缺口明确说明，不虚构外部来源。用户选择的 attachments 必须用于确定范围、重点和练习次序；F 引用表示概念与资料的相关关系，所有资料仍会出现在每个概念中，不要为了可见性给所有概念硬凑相同引用。\n输出 JSON：${STAGED_PLAN_OUTPUT}`,modelInput,(value,prepared)=>settle(value,prepared as typeof modelInput),24576)
  ...
  async direct(ctx:TaskContext,name:string,agent:'L0a'|'A3',input:unknown,angle?:L0aAngle,materialsOnly=false):Promise<string>{
    const depth=ctx.job.input.depth??'fast',budget=materialsOnly?this.capabilities.llm:this.capabilities.zhihu[depth as ThinkingDepth]
    const output=materialsOnly?Math.min(budget.output,depth==='deep'?16384:8192):budget.output
    const system=`${SHARED_SYSTEM_PREFIX}\n${systemPromptFor(agent,angle)}\n如果提供 materials，使用其中与当前概念有关的例子、符号和学习侧重点，不能将整份文件扩展成当前概念之外的讲解。${materialsOnly?'\n当前范围仅限用户选择的收藏和上传文件。只根据本次 materials 整理当前角度；没有依据时说明资料未覆盖，不编造检索、来源或作者。':''}`
    return ctx.step(`${name}@${AGENT_CONTRACT_VERSION}:${digest({system,budget,summary:this.capabilities.llm}).slice(0,16)}`,{input,materialsOnly,budget},async()=>{
      ...
      const messages=await packContext(this.llm,ctx,system,input,depth,{window:budget.window,output,margin:2048,summary:this.capabilities.llm})
      const result=materialsOnly
        ? await this.llm.complete({messages,json:false,thinkingDepth:depth,maxTokens:output,signal:ctx.signal})
