...
export class ZhihuDataClient {
  ...
  constructor(secret:string,fetcher:Fetcher=fetch,base='https://developer.zhihu.com',userMock?:ZhihuOAuthMock,gate?:ZhihuGate){
    if(userMock&&process.env.NODE_ENV==='production')throw new Error('OAUTH_MOCK_FORBIDDEN_IN_PRODUCTION')
    this.gate=gate;this.secret=secret;this.fetcher=fetcher;this.base=base;this.userMock=userMock;this.userMode=userMock?'mock':'real'
  }
  async json(path:string, options:{token?:string;query?:Record<string,string|number|undefined>;body?:unknown;form?:FormData;key?:string;signal?:AbortSignal}={}){
    ...
    return this.request(options.signal,async signal=>{
      ...
      if(options.token&&payload?.Code===20001)throw new ToolError('ZHIHU_AUTH_FAILED',false)
  ...
  async pdfResult(url:string,signal:AbortSignal){
    ...
    return this.request(signal,async signal=>{
      ...
      await this.gate?.observe(response)
      if(!response.ok){await response.body?.cancel();throw new ToolError(response.status===429?'ZHIHU_HTTP_429':'PDF_RESULT_EXPIRED')}
      const max=32*1024*1024;if(Number(response.headers.get('content-length'))>max){await response.body?.cancel();throw new ToolError('PDF_RESULT_TOO_LARGE',false)}
  ...
  private request<T>(signal:AbortSignal|undefined,work:(signal:AbortSignal)=>Promise<T>):Promise<T>{
    return this.gate?this.gate.run(signal,work):work(signal??new AbortController().signal)
  }
}
