...
test('provider capabilities have independent ceilings and production requires all verified limits',()=>{
 const local=resolveCapabilities({});assert.equal(local.llm.window,64000);assert.equal(local.zhihu,undefined)
 const configured={NODE_ENV:'production',DEEPSEEK_CONTEXT_TOKENS:'240000',DEEPSEEK_MAX_OUTPUT_TOKENS:'16000'}
 ...
 assert.equal(resolveCapabilities(configured).llm.window,240000)
 assert.throws(()=>resolveCapabilities({...configured,DEEPSEEK_MAX_OUTPUT_TOKENS:'240000'}),/MODEL_CAPABILITY_CONFIG_INVALID/)
...
test('large and small model windows pack independently; model changes cannot reuse stage output',async t=>{
 ...
 await new ProductTools(llm,{}, {llm:{window:32000,output:8000,namespace:'small-model'}}).structured(ctx,'small-model','system',input,x=>x,1000)
 assert.ok(calls.length>2);assert.equal(input.materials[0].content.length,100000)
