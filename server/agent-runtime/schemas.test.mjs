...
test('extractStructuredJson prefers a route-shaped object over a trailing edge fragment', () => {
  ...
  const formalContent = `先写边。${JSON.stringify(edge)} 再给出完整结构 ${JSON.stringify(route)}`
  assert.deepEqual(extractStructuredJson(formalContent).title, '路线')
  assert.equal(extractStructuredJson(''), undefined)
})
