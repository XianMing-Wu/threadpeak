...
try { await startProductServer() } catch (error) {
  const known=new Set(['PRODUCTION_CONFIG_REQUIRED','MODEL_CAPABILITY_CONFIG_INVALID','LOGIN_URL_MUST_BE_HTTPS','DATABASE_START_FAILED','TRUSTED_PROXY_CONFIG_INVALID','RESOURCE_BUDGET_CONFIG_INVALID'])
