...
6. 创建静态网站并选择已有证书；把 `dist` 内容复制到面板显示的网站运行目录。保留面板管理的证书路径、ACME 规则与 HTTPS 跳转。在该站点 server 块内代理 `/api/` 和精确 `/health` 到 `http://127.0.0.1:4312`；API 使用 HTTP/1.1，转发 Host、实际客户端 IP 和协议，关闭请求/响应缓冲，读写超时 120 秒，上传限制 `101m`。在 `/api/` location 内加入 [JSON 压缩配置](api-json.nginx.conf)，避免每次传输完整未压缩快照；保留 `Cache-Control: no-store`，不启用代理缓存，不压缩 `text/event-stream`。根路径使用 `try_files $uri $uri/ /index.html`，`/assets/` 文件不存在返回 404，缓存一年；HTML 重新验证。更新后先 `nginx -t`，再平滑重载，并用带 `Accept-Encoding: gzip` 的真实快照核对压缩响应与 SSE 首个事件及时到达。
