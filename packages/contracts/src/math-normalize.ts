...
export function decodeMathEncoding(source:string):string{
  return source.replace(/&#(?:92|x5c);|&bsol;|\\u005[cC]/gi,'\\').replace(/\\u0024|&dollar;|&#(?:36|x24);/gi,'$')
    ...
    .replace(/(?:%[0-9a-f]{2}|[a-z0-9_{}^+=().-])+/gi,part=>{if(!/%(?:5c|24)/i.test(part))return part;try{return decodeURIComponent(part)}catch{return part}})
}
...
function closeGroups(tex:string){let balance=0,result='';for(let i=0;i<tex.length;i++){const c=tex[i];if(c==='\\'){result+=c;if(i+1<tex.length)result+=tex[++i];continue}if(c==='{')balance++;if(c==='}'){if(!balance)continue;balance--}result+=c}return result+'}'.repeat(balance)}
export type MathRendering={kind:'rendered';html:string;tex:string;repaired:boolean}|{kind:'unresolved';source:string}
...
export function wrapMathIslands(source:string):string{
  ...
  const flush=()=>{
    ...
    if(text&&!text.includes('%%TPMATH')&&(cue.test(text)||/^[A-Za-z]'$/.test(text))&&!/(?:https?:|[A-Za-z]:\\|`|\b(?:const|function|return)\b)/.test(text))output+=run.replace(text,()=>`$${text.replace(/\\\\$/,'').trim()}$`);else output+=run
    run=''
  }
  for(let i=0;i<source.length;i++){
    ...
    if(depth===0&&(/[\u3400-\u9fff。！？；：，（）\n]/.test(c)||(/[·•・‧]/.test(c)&&(!run.trim()||/\s$/.test(run))))){flush();output+=c}else run+=c
  }
  flush();return output
}
