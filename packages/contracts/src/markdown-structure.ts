...
export function protectMarkdownStructure(source:string,normalize:(text:string)=>string){
  ...
  const saved:string[]=[],edits:Array<{start:number;end:number;value:string}>=[]
  const visit=(node:RootContent,container=false,formatted=false)=>{
    ...
    if(container&&(node.type==='math'||isMathFence)){
      ...
      const token='`'+prefix+saved.length+'`';saved.push(value)
      edits.push({start,end,value:token});return
    }
    ...
    if(['code','inlineCode','inlineMath','math','html','link','image','definition','linkReference','imageReference'].includes(node.type))return
    if(node.type==='text'&&formatted){edits.push({start,end,value:normalize(raw)});return}
    if('children' in node){
  ...
  let text='',cursor=0
  for(const e of edits.sort((a,b)=>a.start-b.start)){if(e.start<cursor)continue;text+=source.slice(cursor,e.start)+e.value;cursor=e.end}
