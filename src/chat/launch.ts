...
import {CHAT_LAUNCH_KEY,clearActiveHistory} from '../history'
...
export function launchChat(query:string,mode:ChatExperience,attachments: PathAttachment[] = [], thinkingDepth: 'fast' | 'deep' = readLearningThinking()) {
  ...
  sessionStorage.setItem(`tp-launch-materials:${conversation.id}`,JSON.stringify(attachments.map(a=>({sourceId:a.sourceId}))))
...
export function newChat(){sessionStorage.removeItem(CHAT_LAUNCH_KEY);clearActiveHistory();location.hash='home'}
