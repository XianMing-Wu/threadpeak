...
| --- | --- |
| Owner | `src/chat/` |
| 新路径 | `resolveOrdinaryAnswer` / `resolveVisualAnswer`：没有真实 R5 / Visualization provider 时显式失败。`resolveChatLaunch`：没有本次发送上下文时进入独立 404，不得预写「性价比高的显卡」或用 localStorage 正文冒充已打开会话 |
