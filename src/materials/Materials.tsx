...
export function MaterialScope({ model }: { model: MaterialsModel }) {
  ...
  return <>
    ...
    {open && createPortal(<div id={id} ref={menu} className="material-scope-menu" style={position} role="menu" aria-label="搜索范围" onKeyDown={event => {
      ...
      {canReadZhihu && <><button type="button" role="menuitem" disabled={model.searchScope.kind==='collections'||!!model.imports['@creation']} onClick={()=>{void model.importCreation();close(true)}}>导入我的公开创作{demo?' · 演示':''}</button><div className="material-scope-divider" role="separator"/>
...
export function MaterialChips({ model }: { model: MaterialsModel }) {
  ...
  return <>
    ...
    {model.imports['@creation']&&<div className="material-inline-notice" role="status"><span>{model.imports['@creation'].error||'正在读取我的公开创作…'}</span>{model.imports['@creation'].error&&<button type="button" onClick={()=>void model.importCreation()}>重试导入创作</button>}<button type="button" onClick={model.removeCreationImport}>移除本次创作导入</button></div>}
...
export function Materials({ model }: { model: MaterialsModel }) {
  const session=useWorkspaceSession()
  const canReadZhihu=session?.provider==='zhihu'&&session.capabilities.zhihuMaterials
  const [saved, setSaved] = useState<Omit<MaterialView,'content'>[]>([]), [notice, setNotice] = useState(''), [loading, setLoading] = useState(false)
  ...
  useEffect(() => {
    ...
    if (!model.preview) {
      ...
      void productRequest<{items:Omit<MaterialView,'content'>[];nextCursor?:string}>('/api/v2/materials').then(page => { if (live){setSaved(page.items.filter(item => item.origin === 'upload'||item.origin==='creation'&&canReadZhihu));setNextCursor(page.nextCursor)} }).catch(e => { if (live) setNotice(e instanceof Error ? e.message : '暂未读取到已保存文件。') }).finally(() => { if (live) setLoading(false) })
    }
    return () => { live = false; dialog.current?.close() }
  }, [open, !!model.preview,canReadZhihu])
  async function loadMore(){if(!nextCursor||loading)return;setLoading(true);try{const page=await productRequest<{items:Omit<MaterialView,'content'>[];nextCursor?:string}>(`/api/v2/materials?cursor=${encodeURIComponent(nextCursor)}`);setSaved(old=>[...old,...page.items.filter(item=>(item.origin==='upload'||item.origin==='creation'&&canReadZhihu)&&!old.some(a=>a.sourceId===item.sourceId))]);setNextCursor(page.nextCursor)}catch(e){setNotice(e instanceof Error?e.message:'暂未读取到文件。')}finally{setLoading(false)}}
  if (!open) return null
  return createPortal(<dialog aria-labelledby={id} className="material-dialog" ref={dialog} onCancel={event => { event.preventDefault(); close() }} onClick={event => { if (event.target === event.currentTarget) close() }}><header><div><h2 id={id}>{preview ? preview.fileName : '已保存的文件'}</h2><p>{preview ? preview.mimeType === 'application/pdf' ? 'PDF 总结 · 本次路线的每个概念都可引用' : '本次路线的每个概念都可引用' : '选择文件，继续用于这次学习。'}</p></div><button type="button" aria-label="关闭资料窗口" onClick={close}><Glyph name="close"/></button></header>
    {preview ? <div className="material-preview">{preview.status === 'ready' ? <>{preview.notice && <p className="material-notice" role="status">{preview.notice}</p>}<MarkdownMath source={preview.content}/></> : <div><p role="status">{preview.notice || preview.job?.phase || '正在整理资料，可关闭窗口继续填写学习目标。'}</p>{['waiting', 'cancelled'].includes(preview.job?.status ?? '') && <button type="button" className="material-retry" onClick={() => void model.resume(preview)}>继续整理</button>}</div>}</div> : <div className="material-dialog-body"><div className="material-saved">{loading && <p role="status">正在读取…</p>}{!loading && !saved.length && <p>上传过的文件会保存在这里，之后可以再次引用。</p>}{saved.map(item => <button type="button" key={item.sourceId} disabled={loading||item.origin==='creation'&&model.searchScope.kind==='collections'||model.files.some(a => a.sourceId === item.sourceId) || model.items.length + model.uploads.length >= 8} onClick={async () => {if(loading)return;setLoading(true);try{model.add(await productRequest<MaterialView>(`/api/v2/materials/${encodeURIComponent(item.sourceId)}`));close()}catch(e){setNotice(e instanceof Error?e.message:'文件暂未读取。')}finally{setLoading(false)} }}><Glyph name="document" size={18}/><span>{item.fileName}<small>{item.status === 'ready' ? item.mimeType === 'application/pdf' ? 'PDF 总结' : '完整正文' : '待继续整理'}</small></span><Glyph name={model.files.some(a => a.sourceId === item.sourceId) ? 'check' : 'plus'} size={16}/></button>)}{nextCursor&&<button type="button" disabled={loading} onClick={()=>void loadMore()}>读取更多资料</button>}</div>{notice && <p className="material-notice" role="alert">{notice}</p>}</div>}
