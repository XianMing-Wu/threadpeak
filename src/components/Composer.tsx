    ...
    <div className="composer-footer">
      <span>
        <div className="composer-thinking" onMouseEnter={() => setMenu('thinking')} onMouseLeave={() => setMenu('')} onFocus={() => setMenu('thinking')} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setMenu('') }}>
          ...
          {menu === 'thinking' && <div className="composer-menu thinking-menu" role="menu">{[['智能思考','智能决策动态搜索'],['深度思考','深入推理给出答案'],['快速回答','跳过推理直达结果']].map(([x,y]) => <button key={x} aria-checked={thinking===x} role="menuitemradio" onClick={() => setThinking(x)}><span><b>{x}</b><small>{y}</small></span>{thinking===x && <Icon name="check" size={15}/>}</button>)}</div>}
        </div>
