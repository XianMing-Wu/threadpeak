...
import {part,lerp} from './learning-motion';
import {LearningCardModel,LearningCardContent,CardAvatar} from './LearningCardModel';
import content,{projectScenario} from './learning-story-content';
import {arrivingAuthors,authorFootprint,bridgeBox,footprintBoxes,footprintEntry,footprintInk,footprintWire,networkVolumeProgress,footprintPaths,networkProgress,projectedNetwork,stageFit} from './author-network-motion';
import './author-network.css';
import {NETWORK_EXIT} from './consultation-motion';
import {AuthorNetwork3D} from './AuthorNetwork3D';
import footprintHeading from './assets/footprint-heading.svg';
import networkHeading from './assets/network-heading.svg';
const featured=content.authorFollowup.paragraphs;

const trimTitle=(s:string)=>s.replace(/\s*-\s*知乎$/,'');
function FootprintMark({kind}:{kind:string}){
...
function Paper({id,w,h,label,children}:{id:string;w:number;h:number;label:string;children:ReactNode}){
 return <svg className="footprint-paper" data-paper={id} width={w} height={h} viewBox={`0 0 ${w} ${h}`} role="group" aria-label={label}>
  ...
  <foreignObject x="1" y="1" width={w-2} height={h-2}><div className="footprint-ink">{children}</div></foreignObject>
 </svg>;
}
export function AuthorNetworkStoryScene({transition}:{transition:RefObject<ScrollTransition>}){
 ...
 useEffect(()=>{
  ...
  const papers=[...el.querySelectorAll<SVGSVGElement>('[data-paper]')];
  ...
  const avatarLocal:Array<{x:number;y:number;w:number}|undefined>=[];
  const reduced=matchMedia('(prefers-reduced-motion: reduce)');
  const resize=new ResizeObserver(()=>{width=el.clientWidth;height=el.clientHeight;avatarLocal.length=0;dirty=true;});resize.observe(el);
  const reflow=()=>{dirty=true;};reduced.addEventListener('change',reflow);
  function opacity(n:SVGElement|HTMLElement,v:number){n.style.opacity=String(v);n.style.visibility=v>.001?'visible':'hidden';}
  function update(){
   const raw=transition.current.progress,p=networkProgress(raw),visible=raw>=27.8&&raw<NETWORK_EXIT;
   el.dataset.active=String(visible);
   if(visible&&!document.hidden&&width){
    if(dirty||lastRaw!==raw||lastSelected!==active.current){
     if(lastSelected&&lastSelected!==active.current&&p>.88&&!reduced.matches){for(const ink of el.querySelectorAll('.footprint-ink'))ink.animate([{opacity:.35,translate:'0 3px'},{opacity:1,translate:'0 0'}],{duration:280,easing:'cubic-bezier(.2,.7,.2,1)'});}
     dirty=false;lastRaw=raw;lastSelected=active.current;
     el.style.opacity='1';el.style.visibility='visible';el.inert=raw>=40.4;el.setAttribute('aria-hidden','false');el.dataset.networkProgress=p.toFixed(5);el.dataset.selectedAuthor=active.current;
     el.style.backgroundColor=`rgb(255 255 255 / ${part(p,0,.14)})`;
     if(p<.30&&active.current!==arrivingAuthors[0])setSelected(arrivingAuthors[0]);
     const fit=stageFit(width,height);stage.setAttribute('viewBox',`0 0 ${fit.W} ${fit.H}`);stage.style.width=`${fit.W*fit.scale}px`;stage.style.height=`${fit.H*fit.scale}px`;stage.style.left=`${fit.x}px`;stage.style.top=`${fit.y}px`;
     stage.dataset.narrow=String(fit.narrow);const nodes=projectedNetwork(p,fit.narrow);
     const area=fit.narrow?{x:20,y:915,w:660,h:470}:{x:744,y:145,w:636,h:600};viewport.style.left=`${fit.x+area.x*fit.scale}px`;viewport.style.top=`${fit.y+area.y*fit.scale}px`;viewport.style.width=`${area.w*fit.scale}px`;viewport.style.height=`${area.h*fit.scale}px`;viewport.style.setProperty('--stage-scale',String(fit.scale));opacity(viewport,networkVolumeProgress(p)>.525?1:0);
     bridges.forEach((n,i)=>{
      const b=bridgeBox(i,p,width,height);n.style.transform=`translate3d(${b.x}px,${b.y}px,0) scale(${b.w/520})`;opacity(n,1-part(p,.275,.35));n.inert=true;
      if(arrivals[i])arrivals[i]!.style.opacity=String(1-part(p,.237,.253));
      n.querySelector<HTMLElement>('.learn-card-ink')!.style.opacity=String(1-part(p,.244,.295));
      // Measure the actual avatar inside the unchanged product card before peeling it away.
      if(!avatarLocal[i]){const avatar=arrivals[i]?.getBoundingClientRect(),root=el.getBoundingClientRect(),scale=b.w/520;avatarLocal[i]=avatar?{x:(avatar.x+avatar.width/2-root.x-b.x)/scale,y:(avatar.y+avatar.height/2-root.y-b.y)/scale,w:avatar.width/scale}:{x:40,y:92,w:31};}
      const av=avatarLocal[i]!;const source={x:b.x+av.x*b.w/520,y:b.y+av.y*b.w/520,size:av.w*b.w/520};
      const dest=nodes.find(v=>v.author?.id===arrivingAuthors[i])!;
      const target=i===0?{x:fit.x+275*fit.scale,y:fit.y+416*fit.scale,size:58*fit.scale}:{x:fit.x+dest.x*fit.scale,y:fit.y+dest.y*fit.scale,size:34*fit.scale};
      ...
      travelers[i].style.width=`${size}px`;travelers[i].style.height=`${size}px`;travelers[i].style.transform=`translate3d(${x-size/2}px,${y-size/2}px,0)`;opacity(travelers[i],v);
     });
     ...
     el.querySelector<SVGGElement>('.network-footnote')!.setAttribute('transform',`translate(${fit.narrow?40:778} ${fit.narrow?1418:786})`);opacity(el.querySelector<SVGGElement>('.network-footnote')!,part(p,.96,.972));
     const hint=el.querySelector<SVGGElement>('.footprint-note')!;opacity(hint,part(p,.83,.84));

    }
   }else if(!visible){el.style.visibility='hidden';el.inert=true;el.setAttribute('aria-hidden','true');lastRaw=-1;}
   frame=requestAnimationFrame(update);
  }
  frame=requestAnimationFrame(update);return()=>{cancelAnimationFrame(frame);resize.disconnect();reduced.removeEventListener('change',reflow);};
 },[transition]);
 return <section ref={host} className="author-network-scene" data-active="false" aria-label="博主网络与学习足迹展示" aria-hidden="true" inert>
  ...
  {featured.map(a=><div className="network-traveler" key={a.id} aria-hidden="true"><CardAvatar src={a.author.avatar} name={a.author.name}/></div>)}
 </section>;
}
