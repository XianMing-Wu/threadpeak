...
import {cycleDialogFocus} from '../learning-v2/dialog-focus';
...
export function LearningProductPrompt({author=false,question,title,onSubmit,onClose,transition}:{transition:RefObject<ScrollTransition>;author?:boolean;question:string;title:string;onSubmit:()=>void;onClose:()=>void}){
 ...
 useEffect(()=>{
  ...
  const tick=()=>{const raw=learningProgress(transition.current.progress),p=media.matches?(chapterStops.find(n=>n>=raw)??2.04):raw,t=Math.max(0,Math.min(1,(p-start)/(end-start)));let budget=t*total,index=0;while(index<chars.length&&budget>=weights[index])budget-=weights[index++];if(t===1||(media.matches&&p>=focus))index=chars.length;
   ...
   if(key!==last){last=key;setState({typed:chars.slice(0,index).join(''),phase,pressure});}frame=requestAnimationFrame(tick);
