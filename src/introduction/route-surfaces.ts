...
export function deckGeometry(a:T.Vector3,b:T.Vector3,width:number,thickness:number,bridge=false,from=0,to=1,startHeight=.523,endHeight=.523){
 ...
 for(let i=0;i<=count;i++){
  ...
  for(const lower of [0,thickness])for(const side of [-1,1])vertices.push(a.x+dx*d+nx*width/2*side,y-lower,a.z+dz*d+nz*width/2*side);
  if(i<count){const n=i*4;top.push(n,n+4,n+1,n+1,n+4,n+5);sides.push(n+2,n+3,n+6,n+3,n+7,n+6,n,n+2,n+4,n+2,n+6,n+4,n+1,n+5,n+3,n+3,n+5,n+7);}
 }
 const last=count*4;sides.push(0,1,2,1,3,2,last,last+2,last+1,last+1,last+2,last+3);
 const indices=[...top,...sides];for(let i=0;i<indices.length;i+=3)[indices[i+1],indices[i+2]]=[indices[i+2],indices[i+1]];
 const g=new T.BufferGeometry();g.setAttribute('position',new T.Float32BufferAttribute(vertices,3));g.setIndex(indices);g.addGroup(0,top.length,0);g.addGroup(top.length,sides.length,1);g.computeVertexNormals();return g;
}
...
export function loopClip(source:T.AnimationClip,duration:number){
 ...
 const tracks=source.tracks.map(sourceTrack=>{
  ...
  for(let i=0;i<track.times.length;i++)if(track.times[i]-start<duration-1e-4){times.push(Math.max(0,track.times[i]-start));for(let j=0;j<size;j++)values.push(track.values[i*size+j]);}
  times.push(duration);values.push(...values.slice(0,size));track.times=new Float32Array(times);track.values=new Float32Array(values);return track;
 });
