export type RouteFrameItem={x:number;y:number;left:number;right:number;top:number;bottom:number;worldRadius?:number};
export type RouteFrameRect={left:number;top:number;right:number;bottom:number};
...
export function fitRouteFrame(items:RouteFrameItem[],rect:RouteFrameRect){
 ...
 for(let i=0;i<40;i++){const mid=(low+high)/2,b=bounds(mid);if(b.right-b.left<=rect.right-rect.left&&b.bottom-b.top<=rect.bottom-rect.top)low=mid;else high=mid;}
 const b=bounds(low);
