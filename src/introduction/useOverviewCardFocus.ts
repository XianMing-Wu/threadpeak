...
import {CARD,learningProgress} from './learning-motion';
...
export function useOverviewCardFocus(host:RefObject<HTMLElement|null>,transition:RefObject<ScrollTransition>){
 useEffect(()=>{
  ...
  const clear=()=>{
   if(active){
    active.style.removeProperty('--hover-x');active.style.removeProperty('--hover-y');active.style.removeProperty('--hover-scale');
    if(reduced.matches)delete active.dataset.hoverLift;else active.dataset.hoverLift='returning';
    active=null;
   }
   delete scene.dataset.hoveredCard;
  };
  const focus=(node:HTMLElement)=>{
   if(!enabled||node.inert||active===node)return;
   const bounds=viewport.getBoundingClientRect(),anchor=node.getBoundingClientRect();
   const scale=anchor.width/CARD.w;if(scale<=0)return;
   ...
   node.dataset.hoverLift='active';scene.dataset.hoveredCard=node.dataset.nodeId;
  };
  const click=(event:MouseEvent)=>{
   ...
   if(node&&scene.contains(node))focus(node);
  };
  const move=(event:PointerEvent)=>{
   ...
   if(!contains(face)&&!contains(active.getBoundingClientRect()))clear();
  };
  const read=(event:Event)=>{const node=event.target instanceof Element?event.target.closest<HTMLElement>('article.learn-object[data-node-id]'):null;if(node)focus(node);};
  const leaveWindow=(event:PointerEvent)=>{if(event.relatedTarget===null)clear();};
  const cleanups=cards.map(node=>{
   const focusOut=(event:FocusEvent)=>{if(active===node&&!node.contains(event.relatedTarget as Node|null))clear();};
   const finish=(event:TransitionEvent)=>{if(event.propertyName==='transform'&&event.target===node.querySelector('.learn-card-model')&&node.dataset.hoverLift==='returning')delete node.dataset.hoverLift;};
   node.addEventListener('focusout',focusOut);node.addEventListener('transitionend',finish);
   return()=>{node.removeEventListener('focusout',focusOut);node.removeEventListener('transitionend',finish);};
  });
  const update=()=>{
   const next=transition.current.progress<27.8&&transition.current.target<27.8&&learningProgress(transition.current.progress)>=1.98&&learningProgress(transition.current.target)>=1.98;
   if(next!==enabled){enabled=next;scene.dataset.overviewInteractive=String(enabled);cards.forEach(node=>{node.tabIndex=enabled?0:-1;});if(!enabled)clear();}
   frame=requestAnimationFrame(update);
  };
  const onKey=(event:KeyboardEvent)=>{
   if(event.key==='Escape'){clear();return;}
   if((event.key==='Enter'||event.key===' ')&&event.target instanceof HTMLElement&&cards.includes(event.target)){event.preventDefault();focus(event.target);}
  };
  const resize=new ResizeObserver(clear);resize.observe(viewport);
  scene.addEventListener('click',click);scene.addEventListener('keydown',onKey);
  scene.addEventListener('learning-card-read',read);
  window.addEventListener('pointermove',move,{passive:true});window.addEventListener('pointerout',leaveWindow);
  window.addEventListener('blur',clear);window.addEventListener('scroll',clear,{passive:true});
  frame=requestAnimationFrame(update);
