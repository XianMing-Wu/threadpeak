...
export function FinaleStoryScene({transition}:{transition:RefObject<ScrollTransition>}){
 ...
 return <section className="finale-scene" ref={host} aria-label="从一个目标、一篇论文或知乎收藏开始学习" data-active="false" aria-hidden="true" inert><div className="finale-stage">
  ...
  <button className="finale-island finale-paper" data-final-part="paper" data-page={page} onClick={()=>setPage(v=>1-v)} aria-label="翻阅 PDF 论文演示"><PaperIllustration page={page}/><span className="finale-island-caption"><strong>从一篇论文开始</strong><span>上传 PDF，把难懂的地方读明白。</span><small>{page===0?'点击，翻到方法页':'点击，返回论文首页'} <b>↗</b></small></span></button>
  <button className="finale-island finale-favorites" data-final-part="favorites" data-collection={collection} onClick={()=>setCollection(v=>1-v)} aria-label="切换知乎收藏夹演示"><FavoritesIllustration collection={collection}/><span className="finale-island-caption"><strong>让收藏，真正被学会</strong><span>沿着自己的知乎收藏，继续探索。</span><small>{collection===0?'点击，换一本收藏':'点击，回到 AI 收藏'} <b>↗</b></small></span></button>
  <button className="finale-island finale-goal" data-final-part="goal" aria-label="只显示目标所需的学习路线" aria-pressed={focused} onClick={()=>setFocused(v=>!v)}><GoalIllustration focused={focused}/><span className="finale-island-caption"><strong>只学，通往目标的那部分</strong><small>{focused?'3 个必要步骤 · 点击还原全貌':'点击，看知识如何变成目标路线'} <b>↗</b></small></span></button>
  ...
  <header className="finale-heading" data-final-part="heading"><span className="finale-eyebrow">问山 · ThreadPeak</span><h2><img src={heading} alt="想做什么，就学什么。"/></h2><p>从你想做成的事出发，定制学习路线。<br/>不必从头啃完整套课程，只学实现目标所需的部分。</p></header>
