...
import {goalIllustrationAt} from './goal-illustration-motion';
import attentionFormula from './assets/attention-formula.svg';
function PaperLines({x,y,width=118,rows=4}:{x:number;y:number;width?:number;rows?:number}){return <g stroke="#cfdaE5" strokeWidth="2.5" strokeLinecap="round">{Array.from({length:rows},(_,i)=><path key={i} d={`M${x} ${y+i*10}h${i===rows-1?width*.7:width}`}/>)}</g>;}
function Drawing({children,kind}:{children:ReactNode;kind:string}){return <svg className={`finale-drawing finale-drawing-${kind}`} viewBox="0 0 360 310" fill="none" aria-hidden="true"><defs><linearGradient id={`finale-paper-${kind}`} x2=".6" y2="1"><stop stopColor="#fff"/><stop offset="1" stopColor="#f8fbff"/></linearGradient><linearGradient id={`finale-edge-${kind}`}><stop stopColor="#b4e8e2"/><stop offset=".5" stopColor="#4d8de4"/><stop offset="1" stopColor="#dcecff"/></linearGradient></defs>{children}</svg>;}
export function PaperIllustration({page}:{page:number}){return <Drawing kind="pdf">
 ...
 <text x="76" y="50" className="finale-art-label">arXiv · 1706.03762</text>
 <g key={page} className="finale-page-content">{page===0?<><text x="76" y="83" className="finale-paper-title">Attention Is</text><text x="76" y="105" className="finale-paper-title">All You Need</text><path d="M76 124H229" stroke="#e1e8f0"/><PaperLines x={76} y={143} width={153} rows={5}/><rect className="finale-highlight" x="72" y="157" width="163" height="15" rx="3" fill="#cfe2ff" opacity=".58"/><PaperLines x={76} y={211} width={146} rows={3}/></>:<><text x="76" y="82" className="finale-paper-title">Scaled Dot-Product</text><text x="76" y="104" className="finale-paper-title">Attention</text><image className="finale-formula" href={attentionFormula} x="72" y="122" width="166" height="62" preserveAspectRatio="xMidYMid meet"/><PaperLines x={76} y={202} width={153} rows={4}/></>}</g>
 <text x="219" y="252" className="finale-art-label">0{page+1}</text></g>
 <g className="finale-annotation"><rect x="215" y="177" width="117" height="69" rx="8" fill="#fff" stroke="#a8c7e9"/><path d="M229 197L234 202L243 190" stroke="#57978a" strokeWidth="1.6"/><text x="249" y="200" className="finale-art-label" fill="#59867e">有据可循</text><path d="M229 216H318M229 227H296" stroke="#d1deec"/><path d={page===0?"M215 196C197 196 197 168 181 168":"M215 225C198 225 203 242 186 242"} stroke="#8ab3e3"/></g>
...
const savedTitles=[['从第一条请求开始','理解模型的上下文','做一个自己的助手'],['Attention 的直觉','把公式变成代码','复现一次小实验']];
export function FavoritesIllustration({collection}:{collection:number}){return <Drawing kind="favorites">
 ...
 <text x="124" y="212" className="finale-folder-title">{collection===0?'我的 AI 收藏':'论文与实践'}</text><text x="124" y="233" className="finale-art-label">收藏的好文章，接着学。</text>
...
export function GoalIllustration({focused}:{focused:boolean}){
 ...
 useEffect(()=>{
  ...
  const from=position.current,start=performance.now(),duration=1500*Math.abs(target-from);let frame=0;
  const update=(value:number)=>{position.current=value;setProgress(value);};
  const tick=(now:number)=>{const t=duration?Math.min(1,(now-start)/duration):1;update(from+(target-from)*(t*t*(3-2*t)));if(t<1)frame=requestAnimationFrame(tick);};
  const preference=()=>{if(media.matches){cancelAnimationFrame(frame);update(target);}};
  if(media.matches)update(target);else frame=requestAnimationFrame(tick);
 ...
 return <Drawing kind="goal">
  ...
  {s.nodes.map((p,i)=><g key={i} className="finale-goal-step" transform={`translate(${p.x} ${p.y})`}>
   ...
   <text y="35" textAnchor="middle" className="finale-step-label">{['调用模型','接入资料','检验回答'][i]}</text>
  </g>)}
  ...
  <g transform="translate(40 14)">
   <rect width="202" height="37" rx="10" fill="#fff" stroke="#b7ceec"/>
   <circle cx="18" cy="18" r="4" fill="#3c80e4"/>
   <text x="31" y="23" className="finale-goal-title">做出自己的问答助手</text>
  </g>
 </Drawing>;
}
