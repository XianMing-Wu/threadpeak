import {SCAN_STOPS} from './story-steps';
...
export const CONSULTATION_START=40.4,CONSULTATION_END=59.5;
export const NETWORK_EXIT=42.25;
export type ConsultBox={x:number;y:number;w:number;h:number};
export type AvatarOrigin={x:number;y:number;size:number};
...
export function consultationAt(raw:number,narrow=false,selected=2){
 const focus=part(raw,48.2,49.45),draft=part(raw,55.3,56.1),entry=part(raw,40.4,42.25);
 const cards=Array.from({length:CONSULTATION_CARD_COUNT},(_,i)=>{
  ...
  return {x:lerp(initial.x,target.x,focus),y:lerp(initial.y,target.y,focus),w:lerp(initial.w,target.w,focus),h:lerp(initial.h,target.h,focus),
   angle:lerp((2.5-i)*3.5,side*-38,focus),z:20-distance,shell:part(raw,41.6+i*.14,42.6+i*.14),ink:part(raw,42.1+i*.14,43.08+i*.14),opacity:1,selected:delta===0};
 });
 ...
 const {x:lensX,y:lensY}=consultationLens(raw,cards);
 return {entry,focus,draft,cards,composer,panel,
  title:part(raw,42.25,43.0),composerIn:part(raw,43.1,43.6)*(1-part(raw,48.1,48.85)),
  question:typeProgress(raw,44.0,45.5),fieldFocus:part(raw,43.84,43.98)*(1-part(raw,45.9,46.12)),
  searchPress:part(raw,45.86,45.95)*(1-part(raw,45.99,46.12)),
  cardPress:part(raw,48.15,48.22)*(1-part(raw,48.27,48.38)),
  preparePress:part(raw,55.05,55.15)*(1-part(raw,55.22,55.35)),
  lens:{x:lensX,y:lensY,opacity:part(raw,43.0,43.65)*(1-part(raw,47.75,48.12))},
  panelIn:part(raw,49.65,49.95),evidenceInk:typeProgress(raw,49.72,50.55),draftInk:typeProgress(raw,56.0,58.0),
  searching:raw>=46.08&&raw<48.1,results:part(raw,47.7,48.2),raw,
 };
}
...
export function consultationPointer(raw:number,narrow:boolean){
 ...
 const a=consultationAt(raw,narrow),b=a.composer,p=a.panel;
 if(raw>=50.65){
  ...
  const press=Math.max(...[51.15,52.15,53.2,54.35,55.15].map(at=>part(raw,at-.10,at)*(1-part(raw,at+.03,at+.19))));
  return{x:lerp(prev.x,next.x,t),y:lerp(prev.y,next.y,t),opacity:part(raw,50.65,50.9)*(1-part(raw,56.15,56.5)),press};
 }
 if(raw>=47.8&&raw<49){const card=a.cards[2],travel=part(raw,47.8,48.12);return {x:card.x+card.w*.58+65*(1-travel),y:card.y+card.h*.47+34*(1-travel),opacity:part(raw,47.8,48.08)*(1-part(raw,48.5,48.85)),press:a.cardPress};}
 const field={x:b.x+85,y:b.y+98},send={x:b.x+b.w-47,y:b.y+b.h-34},prepare={x:p.x+p.w-92,y:p.y+p.h-39},editor={x:p.x+145,y:p.y+191};
 const keys=[{p:43.35,x:b.x-20,y:b.y+156},{p:43.75,...field},{p:45.56,...field},{p:45.86,...send},{p:46.3,...send},{p:50.65,x:p.x+260,y:p.y+250},{p:51.03,...prepare},{p:51.4,...prepare},{p:51.88,...editor},{p:52.35,...editor}];
 const prev=[...keys].reverse().find(k=>k.p<=raw)??keys[0],next=keys.find(k=>k.p>raw)??prev,t=next===prev?0:part(raw,prev.p,next.p);
 const fieldPress=part(raw,43.72,43.79)*(1-part(raw,43.86,43.98));
 return {x:lerp(prev.x,next.x,t),y:lerp(prev.y,next.y,t),opacity:part(raw,43.3,43.55)*(1-part(raw,46.12,46.4))+part(raw,50.55,50.85)*(1-part(raw,52.15,52.5)),press:Math.max(fieldPress,a.searchPress,a.preparePress)};
}
...
export function consultationOrbitCard(index:number,elapsed:number,raw:number){
 ...
 return {x:700+worldX*scale-230*scale/2,y:205+(1-scale)*65,w:230*scale,h:392*scale,angle:-Math.atan(worldX/2800)*180/Math.PI,opacity:1};
}
...
export function consultationLens(raw:number,cards:ConsultBox[]){
 ...
 return {x:lerp(a.x+a.w*.5,b.x+b.w*.5,t),y:lerp(a.y+a.h*.49,b.y+b.h*.49,t)-Math.sin(t*Math.PI)*16,index};
}
