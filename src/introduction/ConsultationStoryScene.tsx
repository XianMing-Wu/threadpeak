...
export function ConsultationStoryScene({transition}:{transition:RefObject<ScrollTransition>}){
 ...
 useEffect(()=>{
  ...
  function tick(now:number){
   ...
   if(width&&height&&!document.hidden){
    ...
    const searchButton=el.querySelector<HTMLElement>('.consult-search')!;searchButton.dataset.busy=String(s.searching);searchButton.setAttribute('aria-label',s.searching?'演示如何核对公开材料':'查看找人示例');
    const status=el.querySelector<HTMLElement>('.consult-search-status')!;status.textContent=s.searching?'演示：结合需求核对公开材料':s.results>.5?'查看适配依据与覆盖边界':'从学习记录里，接着找人';
 ...
 return <section ref={host} className="consultation-scene" data-active="false" aria-label="进一步请教：从具体需求到咨询草稿" aria-hidden="true" inert>
  <div className="consult-stage">
   <header className="consult-heading"><span>进一步请教 · 可选</span><h2><img src={heading} alt="带着问题，寻找合适的人。"/></h2><p>沿路线做出作品，还想听听有经验的人怎么看？<br/>无论是项目评估、审美把关还是亲历经验，都从具体需求出发找人。</p></header>
   ...
   <section className="consult-composer" aria-label="想请教的问题"><div className="consult-settings"><div className="consult-purpose" aria-label="找人的目的"><button type="button" aria-pressed={purpose==='consult'} onClick={()=>setPurpose('consult')}><Mark kind="message"/>咨询人选</button><button type="button" aria-pressed={purpose==='invite'} onClick={()=>setPurpose('invite')}><Mark kind="plus"/>邀请草稿</button></div><span className="consult-learning-check"><Mark kind="check"/>带上学习与实践记录</span></div><div className="consult-question" role="textbox" aria-label="演示输入的问题" aria-readonly="true"><span className="consult-question-text"/><span className="consult-field-placeholder">你具体卡在哪里，想获得什么帮助？</span></div><footer><span className="consult-search-status"/><button className="consult-search" type="button" aria-label="查看找人示例" onClick={()=>seek(50.8)}><Mark kind="arrow"/><span className="consult-search-ellipsis">···</span></button></footer></section>
   <div className="consult-detail">
    <Shell id="detail"/>
    <article className="consult-evidence" aria-label={`${a.name}的推荐依据`}><header><span className="consult-detail-avatar"><CardAvatar name={a.name} src={a.avatar}/></span><div><strong>{a.name}</strong><small>{a.badge}</small></div><span className="consult-match-label">相关切入点</span></header><h3 data-consult-stream data-full={a.lead}/><p className="consult-reason" data-consult-stream data-full={a.summary}/><div className="consult-source-reference"><Mark/><span>{a.title}</span></div><blockquote><small>已保存的公开材料节选</small><p data-consult-stream data-full={a.quote}/></blockquote><p className="consult-limitation" data-consult-stream data-full={a.limitation}/><footer><button className="consult-text-button" onClick={()=>openDetails()}>了解依据与学习足迹 <Mark kind="arrow"/></button><button className="consult-prepare" onClick={prepare}>准备请教 <Mark kind="message"/></button></footer></article>
