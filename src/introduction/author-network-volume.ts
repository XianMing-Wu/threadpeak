...
import {graph,arrivingAuthors} from './author-network-motion';
...
export function networkVolume(){
 ...
 graph.nodes.filter(n=>n.kind==='carrier').forEach((n,i)=>{
  const center=clusters[i];positions.set(n.id,center);
  graph.edges.filter(e=>e.kind==='has-concept'&&e.source===n.id).forEach((e,j)=>{
   const a=j*2.3+i*.8,concept=center.clone().add(V(Math.cos(a)*77,Math.sin(a)*71,(j%2?1:-1)*66));positions.set(e.target,concept);
