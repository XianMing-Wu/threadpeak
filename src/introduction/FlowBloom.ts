...
export class FlowBloom{
 ...
 private scene=new T.Scene();private camera=new T.Camera();private geometry=new T.PlaneGeometry(2,2);
 ...
 private quad=new T.Mesh(this.geometry,this.blur);private width=0;private height=0;
 ...
 render(renderer:T.WebGLRenderer,scene:T.Scene,camera:T.Camera,w:number,h:number,visible:boolean){
  camera.layers.set(0);renderer.setRenderTarget(null);renderer.render(scene,camera);if(!visible)return;
  const width=Math.max(1,Math.round(w)),height=Math.max(1,Math.round(h));
  if(width!==this.width||height!==this.height){this.width=width;this.height=height;for(const target of [this.base,this.scratch,this.inner,this.outer])target.setSize(width,height);}
  const clear=renderer.getClearColor(new T.Color()),alpha=renderer.getClearAlpha();renderer.setClearColor(0,0);
  camera.layers.set(1);renderer.setRenderTarget(this.base);renderer.render(scene,camera);camera.layers.set(0);
  this.quad.material=this.blur;
  for(const [radius,target] of [[7,this.inner],[22,this.outer]] as const){
   this.blur.uniforms.source.value=this.base.texture;this.blur.uniforms.stepSize.value.set(radius/5/width,0);renderer.setRenderTarget(this.scratch);renderer.render(this.scene,this.camera);
   this.blur.uniforms.source.value=this.scratch.texture;this.blur.uniforms.stepSize.value.set(0,radius/5/height);renderer.setRenderTarget(target);renderer.render(this.scene,this.camera);
  }
  renderer.setRenderTarget(null);renderer.setClearColor(clear,alpha);this.quad.material=this.composite;
  const autoClear=renderer.autoClear;renderer.autoClear=false;renderer.render(this.scene,this.camera);renderer.autoClear=autoClear;
 }
 dispose(){for(const t of [this.base,this.scratch,this.inner,this.outer])t.dispose();this.blur.dispose();this.composite.dispose();this.geometry.dispose();}
}
