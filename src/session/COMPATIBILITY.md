...
| --- | --- |
| Owner | `src/session/` |
| 新路径 | `resolveGraphMutation`：我的路线不发明 lesson/图/conversation 节点；KnowledgeCanvas 也不对 mine `growGraph` 或 persist。`resolveAskAuthor`：没有真实 Zhihu-first AskAuthorResolution 时显式失败。`resolveAuthorSearch`：没有真实 network-first AuthorSearch 时显式失败。`resolveAuthorNetwork`：没有真实 relationship projector 时显式失败 |
