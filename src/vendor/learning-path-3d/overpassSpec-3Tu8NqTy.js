...
function mr(e) {
	return H({
		...
		guards: {
			...
			linkedCleanupArmedMatches: ({ context: e, event: t }) => t.type === "ZONE.CLEANUP.ARMED" && t.navigationPlanId === e.plan?.id && X(e.motionCommand, t),
		...
		actions: {
			...
			prepareMotion: V(({ context: e, enqueue: t }) => {
				if (!e.activeIntent || !e.plan) return;
				let n = Qn(e.activeIntent, e.plan, e.stageIndex);
				t.assign({
					motionCommand: n,
					characterCommand: null,
					stageStartedCommitted: !1,
			...
			executePreparedMotion: ({ context: t }) => {
				if (!t.motionCommand) throw Error("Navigation motion must be prepared before it is executed.");
				e.executeMotion(t.motionCommand);
			},
		...
		states: {
			...
			issuingMotion: {
				...
				}, { target: "dispatchingMotion" }]
			},
			armingLinkedCleanup: {
				tags: ["navigation-busy", "navigation-moving"],
				entry: "notifyLinkedCleanup",
				on: { "ZONE.CLEANUP.ARMED": {
					guard: "linkedCleanupArmedMatches",
					target: "dispatchingMotion"
				} }
			},
			dispatchingMotion: {
				tags: ["navigation-busy", "navigation-moving"],
				entry: "executePreparedMotion",
				always: "moving"
			},
			moving: {
				tags: ["navigation-busy", "navigation-moving"],
