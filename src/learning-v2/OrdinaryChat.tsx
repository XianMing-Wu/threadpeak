...
export function OrdinaryChat({chatId,question,initialDepth}:{chatId:string;question:string;initialDepth:'fast'|'deep'}){
  ...
  const accept=(next:Snapshot)=>setSnapshot(old=>old&&old.revision>next.revision?old:next)
  useEffect(()=>{
    const abort=new AbortController();let timer:ReturnType<typeof setTimeout>
    async function poll(id:string){
      try{const next=await productRequest<Snapshot>(`/api/v2/resources/${id}`,{signal:abort.signal});if(!abort.signal.aborted){accept(next);setNotice('')}}catch{if(!abort.signal.aborted)setNotice('正在重新连接，内容仍然保留。')}
  ...
  async function send(){
    if(!snapshot||busy||!value.trim()||lock.current)return
    lock.current=true;setSending(true);const submitted=value
    try{accept(await productRequest(`/api/v2/chats/${snapshot.id}/reply`,{method:'POST',body:{question:submitted.trim(),depth}}));setValue(v=>v===submitted?'':v);setNotice('')}catch(e){setNotice(e instanceof Error?e.message:'暂时未发送，你的输入仍然保留。')}finally{lock.current=false;setSending(false)}
  }
  async function action(name:string){if(!snapshot)return;try{accept(await productRequest(`/api/v2/resources/${snapshot.id}/${name}`,{method:'POST',body:{}}))}catch(e){setNotice(e instanceof Error?e.message:'暂时没有完成。')}}
  return <ProductWorkspace active="paths" page="chat"><main className="query-chat">
    ...
    {(notice||snapshot?.job?.status==='waiting')&&<div className="lp-runtime-notice" role="status">{notice||snapshot?.job?.phase}{snapshot?.job?.status==='waiting'&&<button onClick={()=>void action('resume')}>继续完成</button>}</div>}
