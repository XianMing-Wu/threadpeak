...
import type { AuthorBrief, AuthorMatch, AuthorNetwork, NetworkAuthor, NetworkEvidence } from '../../packages/contracts/src/authors'
import { authorLearningHref, consultationDraft } from '../../packages/contracts/src/authors'
...
export function PrepareAuthor({brief,author,searchId,onClose}:{brief:AuthorBrief;author:AuthorMatch;searchId:string;onClose:()=>void}){
  ...
  const [draft,setDraft]=useState(()=>{try{return sessionStorage.getItem(storageKey)??consultationDraft(brief,author)}catch{return consultationDraft(brief,author)}}),[copied,setCopied]=useState(false),[notice,setNotice]=useState('')
  ...
  return <AuthorDialog title={brief.purpose==='consult'?'准备一份请教简报':'准备知乎提问'} onClose={onClose}><div className="au-dialog-body"><div className="au-person-heading"><AuthorAvatar name={author.authorName} src={author.avatar} sourceUrl={author.url}/><div><h3>{author.authorName}</h3><p>{brief.purpose==='consult'?'先确认服务范围，再决定是否咨询':'用清晰的问题，邀请有相关经验的人'}</p></div></div><div className="au-contact-note"><Glyph name="help"/><p>已找到相关公开材料。{brief.purpose==='consult'?'是否开放咨询、费用和能提供的帮助，需要在知乎与作者确认。':'是否接受邀请、是否回答，由作者决定。'}</p></div><label className="au-field"><span>发送前，按你的情况修改</span><textarea aria-label="请教简报草稿" className="au-draft" value={draft} onChange={e=>{setDraft(e.target.value);setCopied(false)}}/></label><p className="au-muted">这里只有你选入的内容。复制后可自行粘贴到知乎，尚未向任何人发送。</p><div className="au-dialog-actions"><button className="au-primary" disabled={!draft.trim()} onClick={async()=>{try{await navigator.clipboard.writeText(draft);setCopied(true);setNotice('')}catch{setNotice('未能自动复制，请选中上面的文字复制。')}}}><Glyph name={copied?'check':'copy'} size={16}/>{copied?'已复制简报':'复制简报'}</button><a className="au-secondary" href={author.authorUrl||author.url} target="_blank" rel="noopener noreferrer">{author.authorUrl?'去知乎了解作者':'从原文了解作者'}<Glyph name="link" size={15}/></a></div></div></AuthorDialog>
}
type Feedback=(authorId:string,topicId:string,kind:'helpful'|'pinned'|'hidden',value:boolean,evidenceId?:string)=>Promise<void>
function SourceFootprints({evidence,author,onFeedback,busy,onClose}:{evidence:NetworkEvidence;author:NetworkAuthor;onFeedback:Feedback;busy:boolean;onClose:()=>void}){
  const topics=author.topics.filter(t=>evidence.uses.some(u=>u.topicId===t.id))
  return <div className="au-footprints">{topics.map(topic=>{
    const uses=evidence.uses.filter(u=>u.topicId===topic.id),helpful=uses.some(u=>u.helpful)
    return <section key={topic.id}><h4>{topic.title}</h4>{uses.map(use=><div key={use.key}><p>{use.question||`在「${use.topic}」中发现`}</p>{use.resourceId&&<a href={authorLearningHref(use.resourceId)} className="au-link-button" onClick={onClose}>回到这个概念<Glyph name="link" size={12}/></a>}{use.resourceId&&use.nodeIds.length>0&&<details><summary>{use.nodeIds.length} 张关联卡片</summary>{use.nodeIds.map(id=><a href={authorLearningHref(use.resourceId!,id)} key={id} onClick={onClose}>{use.nodeTitles?.[id]??'查看知识卡片'}</a>)}</details>}{use.searchId&&<a className="au-link-button" href={`#authors?search=${encodeURIComponent(use.searchId)}`} onClick={onClose}>查看当时的问题<Glyph name="link" size={12}/></a>}</div>)}<button className="au-link-button" aria-pressed={helpful} disabled={busy} onClick={()=>void onFeedback(author.id,topic.id,'helpful',!helpful,evidence.evidenceId)}><Glyph name={helpful?'check':'like'} size={14}/>{helpful?'已标记对这个主题有帮助':'这篇资料对这个主题有帮助'}</button></section>
  })}</div>
}
export function AuthorDetail({author,onClose,onFeedback,initialLearningId,onImported}:{author:NetworkAuthor;onClose:()=>void;onFeedback:Feedback;initialLearningId?:string;onImported?:()=>void}){
  const [view,setView]=useState<'profile'|'reading'|'connect'|'preferences'>('profile'),[tab,setTab]=useState<'sources'|'history'>('sources')
  const [evidenceId,setEvidenceId]=useState<string>(),[topicId,setTopicId]=useState(initialLearningId&&author.topics.some(t=>t.id===`learning:${initialLearningId}`)?`learning:${initialLearningId}`:author.topics[0]?.id),[busy,setBusy]=useState(false),[notice,setNotice]=useState('')
  const evidence=author.evidence.find(e=>e.evidenceId===evidenceId),topic=author.topics.find(t=>t.id===topicId)??author.topics[0]
  ...
  return <AuthorDialog title={view==='profile'?'作者资料':view==='reading'?'阅读资料':view==='connect'?'用于学习':'推荐设置'} onClose={onClose} onBack={view==='profile'?undefined:back}>
    ...
    <div className="au-dialog-body au-profile-body" key={`${view}:${evidenceId??''}`}>
      {view==='profile'&&<>
        ...
        <nav className="au-profile-nav" aria-label="作者资料视图"><button aria-pressed={tab==='sources'} onClick={()=>setTab('sources')}>资料 <span>{author.evidence.length}</span></button><button aria-pressed={tab==='history'} onClick={()=>setTab('history')}>学习足迹</button><button className="au-preference-entry" onClick={()=>setView('preferences')}>推荐设置<Glyph name="chevron" size={13}/></button></nav>
        {tab==='sources'?<><p className="au-list-caption">在你的学习与找人过程中发现的资料</p><div className="au-author-sources">{author.evidence.map(source=><button className="au-source-row" key={source.evidenceId} onClick={()=>sourceView(source.evidenceId)}><span className="au-source-row-icon"><Glyph name="document" size={18}/></span><span><strong>{source.title}</strong><small>{[...new Set(source.uses.map(u=>u.topic))].join(' · ')||'知乎资料'}</small><SourceMetrics source={source}/></span><Glyph name="chevron" size={16}/></button>)}</div>{!author.evidence.length&&<p className="au-quiet-empty">还没有可阅读的资料。</p>}</>:<><p className="au-list-caption">按资料查看你问过的问题、关联卡片和有帮助反馈。</p>{author.evidence.map(source=><section className="au-history-source" key={source.evidenceId}><button onClick={()=>sourceView(source.evidenceId)} className="au-history-title"><Glyph name="document" size={16}/>{source.title}<Glyph name="chevron" size={14}/></button><SourceFootprints evidence={source} author={author} onFeedback={save} busy={busy} onClose={onClose}/></section>)}</>}
      </>}
      {view==='reading'&&evidence&&<>
        ...
        {evidence.uses.length>0&&<details className="au-reading-footprints"><summary>这篇资料与我的学习 <span>{new Set(evidence.uses.map(u=>u.topicId)).size} 个主题</span></summary><SourceFootprints evidence={evidence} author={author} onFeedback={save} busy={busy} onClose={onClose}/></details>}
      </>}
      {view==='preferences'&&<>
        <h3 className="au-section-title">如何向你推荐 {author.name}</h3><p className="au-muted">偏好按学习主题分别保存。</p>
        {topic?<><div className="au-field"><span>应用到这个主题</span>{author.topics.length===1?<p className="au-single-topic">{topic.title}</p>:<ChoiceMenu label="推荐设置的主题" value={topic.id} disabled={busy} onChange={value=>{setTopicId(value);setNotice('')}} options={author.topics.map(t=>({value:t.id,label:t.title}))}/>}</div>
        <div className="au-preference-options"><label><span><strong>相关时优先推荐</strong><small>寻找「{topic.title}」的博主时，问题适配程度相同的情况下优先推荐这位作者。</small></span><input type="checkbox" role="switch" checked={topic.pinned} disabled={busy||topic.hidden} onChange={e=>void save(author.id,topic.id,'pinned',e.target.checked)}/></label><label><span><strong>暂停这个主题的推荐</strong><small>不再将这位作者推荐给「{topic.title}」的问题。已有资料和学习记录保留。</small></span><input type="checkbox" role="switch" checked={topic.hidden} disabled={busy} onChange={e=>void save(author.id,topic.id,'hidden',e.target.checked)}/></label></div><p className="au-preference-summary">在这个主题中，你主动使用过 {topic.uses} 次，标记过 {topic.helpful} 份资料有帮助。</p></>:<p className="au-quiet-empty">这位作者还没有关联的学习主题。</p>}
      </>}
