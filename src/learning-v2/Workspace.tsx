...
import {cycleDialogFocus,useDialogFocus} from './dialog-focus'
...
export function LearningWorkspace({routeId,conceptId,initialView='research',resourceId,example,onBack}:{routeId:string;conceptId:string;initialView?:'research'|'graph';resourceId?:string;example?:LearningState;onBack?:()=>void}){
  ...
  const [view,setView]=useState(initialView),[presentation,setPresentation]=useState<'map'|'document'>('map'),[depth,setDepth]=useState<'fast'|'deep'>('fast'),[selected,setSelected]=useState<string[]>([]),[detail,setDetail]=useState<string|null>(null)
  ...
  const [editingNodes,setEditingNodes]=useState<GraphNode[]|null>(null),[past,setPast]=useState<EditEntry[]>([]),[future,setFuture]=useState<EditEntry[]>([]),[sending,setSending]=useState(false),[pendingQuestion,setPendingQuestion]=useState('')
  const body=useRef<HTMLDivElement>(null),live=useRef(snapshot),saveChain=useRef(Promise.resolve()),mounted=useRef(true),lastCompleted=useRef(''),sendingRef=useRef(false),unsaved=useRef<GraphNode[]|null>(null),editBase=useRef<GraphNode[]>([])
  const historyDialog=useDialogFocus<HTMLElement>(history)
  ...
  useEffect(()=>{
    ...
    const poll=(id:string)=>{
      const stream=observeResource(id,{signal:abort.signal,afterData:live.current?.dataRevision,onSnapshot:accept})
      return pollResource(async()=>{
      const update=await productRequest<LearningSnapshot|{unchanged:true}>(`/api/v2/resources/${encodeURIComponent(id)}?afterData=${live.current?.dataRevision??0}`,{signal:abort.signal})
      if(abort.signal.aborted)return false
      accept(update)
    },{signal:abort.signal,intervalMs:()=>taskPollInterval(stream.connected()?undefined:live.current?.job?.status),wait:foregroundDelay,onError:(_error,stopped)=>{setOffline(true);if(stopped)setNotice('连接暂停，已有内容仍然保留。点击重新连接继续。')}}).finally(stream.close)
    }
    void (async()=>{
  ...
  useEffect(()=>{
    const followLink=()=>{const node=new URLSearchParams(location.hash.split('?')[1]??'').get('node')
      if(node&&live.current?.data.nodes.some(n=>n.id===node)){setView('graph');setPresentation('map');setFocusNode(node);setSelected([node]);setPhonePane('material')}}
  ...
  const nodes=editingNodes&&state?editPreview(editBase.current,editingNodes,state.nodes):state?.nodes??[]
  ...
  const openArticle=useCallback((id:string)=>{setView('research');setDetail(id);setSelected([id]);setPhonePane('material')},[])
  const openNode=useCallback((id:string)=>{setView('graph');setPresentation('map');setFocusNode(id);setSelected([id]);setPhonePane('material')},[])
  ...
  async function command(kind:string,question?:string,ids=selected){
    ...
    if(kind==='new-conversation'&&(running||job?.status==='waiting')){setNotice('请先停止或完成当前任务，再开始新对话。');return false}
    if(example){
    ...
    sendingRef.current=true;setPendingQuestion(question??'');setSending(true)
  ...
  function saveNodes(next:GraphNode[]){
    ...
    if(!unsaved.current)editBase.current=base
  ...
  const copy=useCallback(async(text:string)=>{try{await navigator.clipboard.writeText(text)}catch{setNotice('可以选中文字手动复制。')}},[])
  const pending:PendingReply[]=(running||job?.status==='waiting')&&job?.kind!=='learning.enter'&&job?.kind!=='learning.import'&&job?.kind!=='learning.materials'&&job?.basisIds?.length===1&&nodes.some(n=>n.id===job.basisIds[0])?[{id:`pending-${job.id}`,mode:job.kind==='learning.author'?'author':'ai',question:'',parents:[job.basisIds[0]!],text:'',status:job.phase,waiting:job.status==='waiting',recoverable:job.recoverable}]:[]
  const phase:Phase=state?.phase??'searching'
  const learningData=useMemo(()=>({articles:state?.articles??[],concept:state?.title??'',example:!!example}),[state?.articles,state?.title,example])
  const composerNodes=useMemo(()=>nodes.filter(n=>!state?.articles.some(a=>a.id===n.id&&a.retainedForHistory)),[nodes,state?.articles])
  ...
  return <LearningData.Provider value={learningData}><div className="lp-workspace">
    <LearningHeader eyebrow={example?'示例学习':'刘看山陪你学'} backLabel={example?'返回上一级':resourceId?'返回知识脉络':'返回3D路线'} view={view} graphReady={nodes.length>0} historyOpen={history} newDisabled={busy} onChange={next=>{setView(next);if(next==='graph')setPresentation('map')}} onBack={onBack??(()=>{location.hash=resourceId?'knowledge':'path-3d'})} onFindPerson={example?undefined:()=>findPerson()} onHistory={()=>setHistory(!history)} onNew={()=>void command('new-conversation')}/>
    {example&&<div className="lp-example-note"><span>示例学习</span><a href="#home">开始我的学习 <Glyph name="chevron" size={13}/></a></div>}
    {(offline||notice||saveState==='error'||job?.status==='waiting')&&<div className="lp-runtime-notice" role="status"><span>{notice|| (saveState==='error'?'卡片编辑尚未保存，请选择处理方式。':offline?'正在重新连接，内容仍然保留。':job?.phase)}</span>{job?.status==='waiting'&&<>{job.recoverable&&<button onClick={()=>void taskAction('resume')}>继续完成</button>}<button onClick={()=>void taskAction('cancel')}>停止本次任务</button></>}{unsaved.current&&<><button onClick={()=>{const next=editPreview(editBase.current,unsaved.current!,live.current!.data.nodes);unsaved.current=null;saveNodes(next)}}>保留当前版本</button><button onClick={()=>{unsaved.current=null;setEditingNodes(null);setSaveState('idle');setNotice('')}}>使用已保存版本</button></>}{offline&&<button onClick={()=>{setNotice('');setReconnect(n=>n+1)}}>重新连接</button>}{notice&&<button onClick={()=>setNotice('')} aria-label="关闭提示">×</button>}</div>}
    ...
    <main className="lp-body" ref={body} style={{'--lp-split':`${split}%`} as React.CSSProperties} data-phone-pane={phonePane}>
      <section className="lp-material-pane"><div className="lp-material-view" hidden={view!=='research'}><ArticlePanel onAuthor={example?undefined:id=>findPerson(id)} phase={phase} detail={detail} selected={selected} onOpen={openArticle} onBack={()=>setDetail(null)} onToggle={id=>selectNode(id,true)} canRetry={!!job?.recoverable} onRetry={()=>void taskAction('resume')}/></div>
        {!!nodes.length&&<div className="lp-material-view" hidden={view!=='graph'}>{saveState!=='idle'&&<div className="lp-save-status" role="status"><StatusPill busy={saveState==='saving'}>{saveState==='saving'?'正在保存卡片':saveState==='saved'?'卡片已保存':'卡片尚未保存'}</StatusPill></div>}<KnowledgeGraph active={view==='graph'} presentation={presentation} onPresentation={setPresentation} nodes={nodes} selected={selected} onSelect={selectNode} onSelection={setSelected} onPromptSubmit={(q,mode,ids)=>busy?Promise.resolve(false):command(mode==='author'?'author':'reply',q,ids)} submitError={notice} pending={pending} onChange={changeGraph} onUndo={undo} onRedo={redo} canUndo={!!past.length} canRedo={!!future.length} onStop={()=>void taskAction('cancel')} onResume={()=>void taskAction('resume')} depth={depth} onDepth={setDepth} busy={busy} onCopy={copy} onSource={openArticle} focusId={focusNode}/></div>}
      </section><PaneDivider split={split} onChange={setSplit} container={body}/>
      <ChatPanel sending={sending} pendingQuestion={pendingQuestion} task={job?.conversationId===state.active?job:undefined} paused={job?.status==='waiting'||(running&&job?.conversationId!==state.active)} conversation={conversation} selected={selected} nodes={composerNodes} depth={depth} onDepth={setDepth} onRemove={id=>setSelected(s=>s.filter(n=>n!==id))} onClear={()=>setSelected([])} onSend={q=>command('reply',q)} onStop={()=>void taskAction('cancel')} mode="ai" onMode={()=>{}} focusToken={0} phase={running?phase:phase==='empty'?'empty':'ready'} draft={job?.conversationId===state.active?job?.draft??'':''} busy={sending||(running&&job?.conversationId===state.active)} authorBusy={job?.kind==='learning.author'&&running} onSource={openArticle} onNode={openNode} onCopy={copy} onRetry={()=>void taskAction('resume')}/>
    </main>
    {history&&<><button className="lp-overlay-scrim" aria-label="关闭对话历史" onClick={()=>setHistory(false)}/><aside ref={historyDialog} role="dialog" className="lp-history lp-enter" aria-label="对话历史" onKeyDown={event=>{cycleDialogFocus(event);if(event.key==='Escape')setHistory(false)}}><header><h2>这个概念的对话</h2><IconButton icon="close" label="关闭历史" onClick={()=>setHistory(false)}/></header><p>对话分别保存，知识脉络持续积累。</p>{[...state.conversations].reverse().map(c=><button className={`lp-history-item ${c.id===state.active?'is-active':''}`} key={c.id} onClick={async()=>{if(example){saveExample({...state,active:c.id});setHistory(false);setSelected([]);return}try{accept(await productRequest(`/api/v2/learning/${snapshot!.id}/commands`,{method:'POST',body:{kind:'activate-conversation',conversationId:c.id,depth}}));setHistory(false);setSelected([])}catch(e){setNotice(e instanceof Error?e.message:'暂时未打开。')}}}><Glyph name="message" size={17}/><span><strong>{c.title}</strong><small>{c.messages.filter(m=>m.role==='user').length} 个问题</small></span></button>)}</aside></>}
