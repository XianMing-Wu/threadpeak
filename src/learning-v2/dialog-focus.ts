...
export function useDialogFocus<T extends HTMLElement>(open=true){
  ...
  useEffect(()=>{if(!open)return;const opener=document.activeElement as HTMLElement|null;ref.current?.querySelector<HTMLElement>('button:not(:disabled),textarea,input')?.focus();return()=>{if(opener?.isConnected)opener.focus()}},[open])
  return ref
}
