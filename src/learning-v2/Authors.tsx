...
import { rankAuthorMatches, type AuthorBrief, type AuthorMatch, type AuthorNetwork, type AuthorSearchState } from '../../packages/contracts/src/authors'
...
export function DurableAuthorsPage(){
  ...
  useEffect(()=>{
    ...
    const poll=async()=>{
      try{const next=await productRequest<Snapshot>(`/api/v2/resources/${encodeURIComponent(resourceId)}`,{signal:abort.signal});if(abort.signal.aborted)return
        setSnapshot(next)
        ...
        if(['queued','running'].includes(next.job?.status??''))timer=setTimeout(()=>void poll(),850)
        else{void loadNetwork().catch(()=>{});void history()}
      }catch(e){if(!abort.signal.aborted){setNotice(e instanceof Error?e.message:'正在重新连接。');timer=setTimeout(()=>void poll(),2000)}}
    };void poll();return()=>{abort.abort();clearTimeout(timer)}
  },[resourceId,loadNetwork,history,snapshot?.job?.status==='waiting'])
  ...
  return <ProductWorkspace active="authors" page="authors"><main className="ux-flowith peak-market au-page" tabIndex={0} aria-label="博主搜索与网络">
    ...
    {notice&&<div className="au-notice" role="status"><span>{notice}</span><button onClick={()=>void loadNetwork().then(()=>setNotice('')).catch(e=>setNotice(e.message))}>重新连接</button><button aria-label="关闭提示" onClick={()=>setNotice('')}><Glyph name="close" size={14}/></button></div>}
