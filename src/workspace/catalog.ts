...
export function defaultAnswerMock(query: string) {
  if (/显卡|gpu|游戏性能|性价比/i.test(query)) {
    return {
      heading: '先按使用场景筛选，而不是只看一张性能榜',
      lead: '“性价比”必须同时考虑目标分辨率、常玩游戏、整机功耗和预算。下面的分档是交互 Mock，用来展示回答层级，不代表实时购买报价。',
      sections: [
        ['1080p 实用档', '优先确认显存余量与常玩游戏的实际帧率，不为暂时用不到的高分辨率性能多付预算。'],
        ['2K 均衡档', '把平均帧率、最低帧率、功耗与散热一起比较，避免只看单项峰值。'],
        ['生产力兼顾档', '先核对软件生态、编码器与显存需求，再比较游戏性能。'],
      ],
    }
  }
  if (/傅里叶|fft|频谱/i.test(query)) {
    return {
      heading: '先把信号看成一堆不同频率的波，再谈公式',
      lead: `你问的是「${query}」。这一答只处理频域直觉，不会把它偷偷换成一条学习路线。`,
      sections: [
        ['时域看到的是叠加', '你听到或看到的波形，是许多不同频率振动叠在一起的结果。'],
        ['频域把它们分开', '傅里叶变换 $\\hat{f}(\\xi)=\\int_{-\\infty}^{\\infty} f(x)\\,e^{-2\\pi i x\\xi}\\,dx$ 回答的是：这些振动各自有多强，相位如何。'],
        ['下一步通常是例子', '用一条由两个正弦波合成的曲线，比直接写积分定义更接近“理解了”。'],
      ],
    }
  }
  if (/产品经理|数据分析/i.test(query)) {
    return {
      heading: '先会提出能被数据否定的问题，再学工具',
      lead: `围绕「${query}」，优先建立指标和对比，而不是先堆仪表盘。`,
      sections: [
        ['先写决策', '这次分析结束时，你要改变哪个产品判断？'],
        ['再选指标', '指标必须能随这个判断一起对或错。'],
        ['最后才是工具', '表格、SQL 或图表只服务上面两个问题。'],
      ],
    }
  }
  return {
    heading: '先把问题拆成可以判断的几部分',
    lead: `我会按「${query}」本身来组织，而不是套用另一道题的现成回答。`,
    sections: [
      ['先明确目标', '你最终想得到解释、决策建议，还是一条可以执行的步骤？'],
      ['再补关键限制', '预算、时间、已有基础会直接改变答案。'],
      ['最后给行动项', '把结论压缩成下一步可以直接执行的选择。'],
    ],
  }
}
