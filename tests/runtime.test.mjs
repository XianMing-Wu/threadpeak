...
test('博主搜索支持放大镜展开临时输入、每次重新搜索，且缺 provider 显式失败',()=>{assert.match(authors,/openSearch/);assert.match(authors,/setSearchOpen\(true\)/);assert.match(authors,/submitSearch/);assert.match(authors,/setSearchOpen\(false\)/);assert.match(authors,/setQuery\(''\)/);assert.match(authors,/resolveAuthorSearch/);assert.match(authors,/AuthorSearchUnavailable/);assert.match(authors,/>搜索博主</);assert.doesNotMatch(authors,/runAuthorGraphRag|startRadar|setInterval|setRevealedCount|changeGoal|changeView|hideAuthor|restoreHidden|setAuthorFeedback/)})
