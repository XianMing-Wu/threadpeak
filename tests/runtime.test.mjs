...
test('问题会话支持普通回答与路线制定两种入口',()=>{assert.match(chat,/resolveOrdinaryAnswer/);assert.match(chat,/OrdinaryAnswerUnavailable/);assert.match(chat,/ChatRoutePanel/);assert.match(chat,/followUp/);assert.match(chat,/resolveChatLaunch/);assert.doesNotMatch(chat,/resolveVisualAnswer|VisualAnswerUnavailable|图文模式/);assert.doesNotMatch(chat,/DefaultAnswer|defaultAnswerMock|function VisualAnswer\b|selectVisualFrames|性价比高的显卡/)})
test('学习追问必须有问题且不再提供图文模式',()=>{assert.match(session,/resolveFollowUpHost/);assert.match(session,/requestFollowUp/);assert.doesNotMatch(session,/图文模式|resolveVisualAnswer|mode-prompts/)})
test('新对话清空当前会话并形成后续树分支入口',()=>{assert.match(session,/setTurns\(\[\]\)/);assert.match(session,/new-chat/)})
test('博主搜索支持放大镜展开临时输入、每次重新搜索，且搜索与网络缺 provider 显式失败',()=>{assert.match(authors,/openSearch/);assert.match(authors,/setSearchOpen\(true\)/);assert.match(authors,/submitSearch/);assert.match(authors,/setSearchOpen\(false\)/);assert.match(authors,/setQuery\(''\)/);assert.match(authors,/resolveAuthorSearch/);assert.match(authors,/AuthorSearchUnavailable/);assert.match(authors,/resolveAuthorNetwork/);assert.match(authors,/AuthorNetworkUnavailable/);assert.match(authors,/label: '搜索博主'/);assert.match(authors,/label: '博主网络'/);assert.match(authors,/AuthorNetworkGraph/);assert.match(authors,/projectAuthorNetworkGraph/);assert.doesNotMatch(authors,/useAuthorNetwork|hydrateNetworkFromAnnotations|runAuthorGraphRag|startRadar|setInterval|setRevealedCount|changeGoal|changeView|hideAuthor|restoreHidden|setAuthorFeedback/)})
