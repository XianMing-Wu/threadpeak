...
test('博主搜索支持放大镜展开临时输入、每次重新搜索，且搜索与网络缺 provider 显式失败',()=>{assert.match(authors,/openSearch/);assert.match(authors,/setSearchOpen\(true\)/);assert.match(authors,/submitSearch/);assert.match(authors,/setSearchOpen\(false\)/);assert.match(authors,/setQuery\(''\)/);assert.match(authors,/resolveAuthorSearch/);assert.match(authors,/AuthorSearchUnavailable/);assert.match(authors,/resolveAuthorNetwork/);assert.match(authors,/AuthorNetworkUnavailable/);assert.match(authors,/>搜索博主</);assert.match(authors,/>博主网络</);assert.doesNotMatch(authors,/AuthorNetworkGraph|useAuthorNetwork|hydrateNetworkFromAnnotations|runAuthorGraphRag|startRadar|setInterval|setRevealedCount|changeGoal|changeView|hideAuthor|restoreHidden|setAuthorFeedback/)})
...
test('从学习页进入画布时右上角是回到对话，从知识脉络进入不显示新对话',()=>{assert.match(canvas,/CanvasConversationAction/);assert.match(canvas,/<CanvasConversationAction returnTo=\{returnTo\} routeId=\{knowledge\?\.routeId \?\? ''\} conceptId=\{conceptId\}\/>/);assert.doesNotMatch(canvas,/新对话/)})
