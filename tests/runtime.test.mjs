...
test('问题会话支持普通回答、路线澄清与相关图文三种入口',()=>{assert.match(chat,/resolveOrdinaryAnswer/);assert.match(chat,/OrdinaryAnswerUnavailable/);assert.match(chat,/ChatRoutePanel/);assert.match(chat,/VisualAnswer/);assert.match(chat,/setTimeout/);assert.match(chat,/followUp/);assert.doesNotMatch(chat,/DefaultAnswer|defaultAnswerMock/)})
