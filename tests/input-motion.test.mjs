...
test('the reference orbit is shared by the product and path-lab entries', () => {
  ...
  assert.match(motion, /padding: 28px/)
  assert.match(motion, /filter: blur\(16px\) saturate\(1\.08\)/)
  assert.match(motion, /\.input-motion-frame > \* \{/)
  assert.match(motion, /z-index: 1/)
  assert.doesNotMatch(motion, /z-index: -1/)
  assert.match(motion, /\.input-motion-frame::before,\s*\.input-motion-frame::after \{[\s\S]*conic-gradient\(\s*from var\(--tp-input-motion-angle\)/)
