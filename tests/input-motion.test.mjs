...
test('the reference orbit is a path-local radial leak, not a conic ribbon', () => {
  ...
  assert.match(motion, /animation: tp-input-motion-orbit 2\.2s linear infinite/)
  assert.match(motion, /offset-path: inset\(0 round var\(--tp-input-motion-radius\)\)/)
  assert.match(motion, /offset-distance/)
  assert.match(motion, /radial-gradient\(/)
  assert.match(motion, /width: 24px/)
  assert.match(motion, /width: 68px/)
  ...
  assert.doesNotMatch(motion, /--tp-input-orb-x/)
