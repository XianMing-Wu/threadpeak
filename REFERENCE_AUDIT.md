...
| `src/pages/Session.tsx`、`src/session/`、`src/knowledge-canvas/`、`src/workspace/nav.ts` | 未选择时不再默认线性代数；我的路线不再 `draftFirstLesson` 建图；示例仍读标记 catalog lesson | canonical 初始回复 settle 后由 GraphSurgeon 创建并增量更新 |
...
| 博主搜索 | 当前用户网络优先；有 1–3 位即停止；0 位才查知乎并由 LLM 选最多 3 位 | 当前本地检索仅是视觉原型 |
...
当前源码中的 `defaultAnswerMock`、catalog lesson/route、固定作者、本地 GraphRAG、页面 timer 和浏览器事实源都是阻断 `integrated` 的迁移债务。它们可以暂时支撑视觉验收，但必须从用户请求的生产 composition 中删除。

## 博主搜索与网络视觉锚点

| 可见合同 | 当前 UI 证据 | 目标数据语义 |
| --- | --- | --- |
| 搜索过程分阶段可见 | `radar-search-trigger`、`radar-sweep`、`revealedCount` | 先 network stage，零命中后才 Zhihu stage |
| 一位作者只出现一次 | `consultation-advisor`、标签环 | 以稳定知乎外部用户 ID 去重，不按显示名合并 |
| 结果可解释 | ranking/detail 卡 | 显示来源、相关性、freshness 与真实内容链接 |
