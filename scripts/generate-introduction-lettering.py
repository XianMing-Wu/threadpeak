...
labels=[('footprint-heading','记住问题，也记住帮过你的人。',5),('network-heading','遇到难题，知道向谁请教。',5),('consultation-heading','AI 解不开，向行家请教。',7),('finale-heading','想做什么，就学什么。',5)]
for name,label,blue_from in labels:
 x=0;paths=[];bounds=[]
 for i,char in enumerate(label):
  use_latin=name=='consultation-heading' and i<2
...
(root/'src/introduction/assets/story-lettering-source.json').write_text(json.dumps({'family':'Ma Shan Zheng','latin':'KaTeX Main Bold, upright outlined AI capitals','license':'SIL OFL 1.1','sha256':digest,'labels':{name:label for name,label,_ in labels}},ensure_ascii=False,indent=2)+'\n')
