...
[在线体验](https://threadpeak.wuxianming.ac.cn/) · [快速开始](#快速开始) · [核心能力](#核心能力) · [开发](#开发) · [文档](docs/README.md) · [部署](deploy/README.md)

![问山首页：描述学习目标并选择资料与检索范围](docs/images/home-2026-09.png)

*2026 年 9 月界面实拍。可以使用知乎账号登录，也可以先以游客身份体验。*
...
- **控制本次学习的材料**：支持 PDF、Markdown、TXT，以及显式导入的知乎收藏夹和公开创作。收藏文章按概念与目标筛选，每个概念只展示相关资料；仅收藏夹范围不会额外搜索外部内容。
- **找到问题背后的作者**：按问题检索公开文章和相关博主，保留来源关系与主题反馈，帮助准备请教。这里提供的是资料与线索，不代表作者本人在线回复。
- **中断后继续**：任务、检查点、对话和知识保存在服务端。刷新页面不取消生成；新对话保留已有文章和知识树。
- **统一阅读体验**：回答逐步显示，思考结束后自动收起；发送操作即时反馈。聊天、文章、卡片和文档共用 Markdown、代码与数学公式渲染，原始材料与编辑副本分开保存。

![真实生成的梯度下降路线，包含可进入的概念节点和上次学习位置](docs/images/route-2026-09.png)

*由“用一个数字例子理解梯度下降的一次参数更新”生成的路线。短窗口也可浏览，概念卡片采用紧凑字号，节点可以随时进入；具备独立学习内容的路线会按实际依赖并行与汇合。*
...
当前提供线上体验，已在 2 核、4 GB VPS 上部署，并执行真实知乎收藏夹、PDF、模型回答和 PostgreSQL 验证。在线阅读与模型生成分别控制并发；长时间运行、上游服务额度及数据保留政策仍需持续完善，详见[现状与边界](docs/product.md#42-仍需实际验收或外部配置)。测试结果须结合具体版本与负载范围理解。
